...
#=╠═╡
const test_sarsa_fcann_output = run_sarsa_rubiks_fcann_test(3, 3, sarsa_params_layers, 0.0f0; max_steps = 1_000, α = 1f-2, ϵ = 0.01f0, parameters = sarsa_fcann_params) # feature_vector = make_rubiks_feature(solved_cube_indices))
  ╠═╡ =#
...
#=╠═╡
const test_dp_fcann_output = run_dp_rubiks_fcann_test(4, 5, dp_params_layers; max_steps = 1_000, α = 1f-4, ϵ = 0.01f0, parameters = dp_fcann_params) #, feature_vector = make_rubiks_feature(solved_cube_indices))
  ╠═╡ =#
...
const dp_mastery_params_layers = fill(256, 4)
...
# ╠═╡ disabled = true
#=╠═╡
const step_mastery_dp_fcann_output = run_dp_step_mastery_fcann_test!(dp_mastery_fcann_params, 6, 7, deepcopy(rubiks_binary_feature), update_rubiks_feature!, 1_000_000; α = 2f-4, ϵ = 0.01f0)
  ╠═╡ =#
...
begin
	...
	@bind dp_fcann_step_mastery_layer_select Select(collect(keys(dp_fcann_step_mastery_results)))
end
  ╠═╡ =#
...
# ╔═╡ 6af55326-c493-4b83-90eb-b75f046ba091
md"""
### State MDP Version of Pocket Cube
"""

# ╔═╡ 2abaa0ec-9173-481c-8b6c-69d8444e716a
function score_pocket_cube(s::AbstractVector{I}) where I <: Integer
	f = 0
	@inbounds @simd for i in 1:24
		f += (s[i] == solved_pocket_cube[i])
	end
	return f/24f0
end

# ╔═╡ 96fa5c6f-a968-4999-8764-bcb705680e3c
#add n-step move versions of MDP to be evaluated by linear policy gradient

# ╔═╡ b92e3ada-6bb5-4936-b58e-af64e37018cd
md"""
#### Linear Parameter Studies
"""

# ╔═╡ f6fbf5c2-6fa9-4977-8fa7-399f99b84ef0
#=╠═╡
function run_linear_sarsa_value_study(study, αs::Vector, λs::Vector, ϵs::Vector, num_steps::Integer; kwargs...)
	for α in αs for λ in λs for ϵ in ϵs
		study.update_results!(α, λ, num_steps; ϵ = ϵ, kwargs...)
		study.update_results!(α, λ, num_steps; compute_value = compute_q_learning_value, ϵ = ϵ, kwargs...)
		study.update_results!(α, λ, num_steps; compute_value = compute_expected_sarsa_value, ϵ = ϵ, kwargs...)
	end end end

	df = DataFrame((;k..., output = study.results[k]) for k in keys(study.results))
	sort(df, :output; rev = true)
end
  ╠═╡ =#

# ╔═╡ e32c68e7-b828-4e52-9b1b-308a8570e554
#=╠═╡
function run_linear_dp_value_study(study, αs::Vector, λs::Vector, ϵs::Vector, num_steps::Integer; kwargs...)
	for α in αs for λ in λs for ϵ in ϵs
		study.update_results!(α, λ, num_steps; ϵ = ϵ, kwargs...)
	end end end
...
# ╔═╡ bd144183-27bd-4de0-8d33-f5f0b9fb4d41
#=╠═╡
function run_linear_policy_study(study, αθs::Vector, αws::Vector, λθs::Vector, λws::Vector, num_steps::Integer; kwargs...)
	for α_θ in αθs for α_w in αws for λ_θ in λθs for λ_w in λws
		study.update_results!(α_θ, α_w, λ_θ, λ_w, num_steps; kwargs...)
	end end end end
...
# ╔═╡ 7222c4ec-9473-4b25-97d2-3349a9118c8a
# ╠═╡ disabled = true
#=╠═╡
run_linear_sarsa_value_study(pocket_cube_curriculum_studies1.value_studies.sarsa_linear_study, 2f0 .^(-6:-4), [0f0, 0.1f0, 0.5f0], [0.01f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ 9c22ff39-d25c-4a1b-9487-fbe0e1c8c5cd
# ╠═╡ disabled = true
#=╠═╡
run_linear_dp_value_study(pocket_cube_curriculum_studies1.value_studies.dp_linear_study, 2f0 .^(-10:-4), [0f0, 0.1f0, 0.2f0, 0.5f0], [0.001f0, 0.01f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ 05321eff-d482-4ef9-bac9-cf2508950228
# ╠═╡ disabled = true
#=╠═╡
run_linear_policy_study(pocket_cube_curriculum_studies1.policy_studies.ac_linear_study, 2f0 .^(-6:-4), 2f0 .^(-9:-6), [0f0, .1f0, 0.2f0], [0f0, .1f0, 0.2f0, 0.5f0], 100_000; α_r̄ = 0.01f0)
  ╠═╡ =#

# ╔═╡ 5818f1e0-0430-4d2e-a748-3ebcaa3f7bf4
# ╠═╡ disabled = true
#=╠═╡
run_linear_sarsa_value_study(pocket_cube_curriculum_studies2.value_studies.sarsa_linear_study, 2f0 .^(-6:-4), [0f0, 0.1f0, 0.5f0], [0.01f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ b296bd10-a076-4735-9c9c-5b9d42ca0911
# ╠═╡ disabled = true
#=╠═╡
run_linear_dp_value_study(pocket_cube_curriculum_studies2.value_studies.dp_linear_study, 2f0 .^(-10:-6), [0f0, 0.1f0, 0.2f0, 0.5f0, 0.6f0, 0.7f0, 0.8f0, 0.9f0], [0.001f0, 0.01f0, 0.05f0], 100_000; α_r̄ = 0.01f0)
  ╠═╡ =#

# ╔═╡ 77936d85-f2a3-4c50-8ac4-6227f2f89aee
# ╠═╡ disabled = true
#=╠═╡
run_linear_policy_study(pocket_cube_curriculum_studies2.policy_studies.ac_linear_study, 2f0 .^(-4:-2), 2f0 .^(-8:-5), [0f0, .1f0, 0.2f0, 0.5f0, 0.9f0], [0f0, .1f0, 0.2f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ 5f756533-50ee-4564-b4e8-46ba725fd25d
# ╠═╡ disabled = true
#=╠═╡
run_linear_dp_value_study(pocket_cube_curriculum_studies3.value_studies.dp_linear_study, 2f0 .^(-11:-8), [0.5f0, 0.6f0, 0.7f0, 0.8f0], [0.001f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ dbc3ec20-d7a7-409f-b9a1-f9332be43626
# ╠═╡ disabled = true
#=╠═╡
run_linear_policy_study(pocket_cube_curriculum_studies3.policy_studies.ac_linear_study, 2f0 .^(-4:-3), 2f0 .^(-8:-5), [0f0, 0.1f0, 0.5f0, 0.6f0], [0f0, 0.1f0, 0.5f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ 1d01dd20-10d7-4561-8f00-53c0e6f991a7
md"""
#### Non-linear Parameter Studies
"""

# ╔═╡ fac6f254-93da-4912-a67b-0c9f0fcea252
# ╠═╡ disabled = true
#=╠═╡
function run_nonlinear_dp_value_study(study, αs::Vector, λs::Vector, ϵs::Vector, num_steps::Integer; layer_size = 4, num_layers = 2, reslayers = 1, kwargs...)
	for α in αs for λ in λs for ϵ in ϵs
		study.update_results!(α, λ, num_steps, layer_size, num_layers, reslayers; ϵ = ϵ, kwargs...)
	end end end
...
# ╔═╡ a24d2e2c-6414-4213-894e-990626b43d9e
# ╠═╡ disabled = true
#=╠═╡
function run_nonlinear_policy_study(study, αθs::Vector, αws::Vector, λθs::Vector, λws::Vector, num_steps::Integer; layer_size = 4, num_layers = 2, reslayers = 1, kwargs...)
	for α_θ in αθs for α_w in αws for λ_θ in λθs for λ_w in λws
		study.update_results!(α_θ, α_w, λ_θ, λ_w, num_steps, layer_size, num_layers, reslayers; kwargs...)
	end end end end
...
# ╔═╡ 31a07848-e8ba-4f68-80bf-a10cbb230cac
#=╠═╡
run_nonlinear_dp_value_study(pocket_cube_curriculum_studies1.value_studies.dp_nonlinear_study, 2f0 .^(-10:-4), [0f0, 0.1f0, 0.2f0, 0.5f0], [0.001f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ b5dfd3d6-7086-47f1-8d14-21aea221cf83
#=╠═╡
run_nonlinear_dp_value_study(pocket_cube_curriculum_studies2.value_studies.dp_nonlinear_study, 2f0 .^(-10:-2), [0f0, 0.1f0, 0.2f0, 0.5f0, 0.9f0], [0.001f0], 100_000; α_r̄ = 0.1f0)
  ╠═╡ =#

# ╔═╡ 41bd044b-bf7d-4e52-8f1c-74a514b827fb
#=╠═╡
run_nonlinear_dp_value_study(pocket_cube_curriculum_studies3.value_studies.dp_nonlinear_study, 2f0 .^(-10:-2), [0f0, 0.1f0, 0.2f0, 0.5f0, 0.9f0], [0.001f0], 100_000; α_r̄ = 0.1f0, layer_size = 16, num_layers = 4)
  ╠═╡ =#

# ╔═╡ b034615e-25e4-4207-8fe8-e165cbbb4894
#=╠═╡
run_nonlinear_policy_study(pocket_cube_curriculum_studies1.policy_studies.ac_nonlinear_study, 2f0 .^(-5:-3), 2f0 .^(-6:-4), [0f0, 0.1f0, 0.5f0, 0.9f0], [0f0, 0.1f0, 0.5f0, 0.9f0], 100_000; α_r̄ = 0.1f0, layer_size = 16, num_layers = 4)
  ╠═╡ =#

# ╔═╡ f02298a1-798d-403c-bf84-35188191e871
#=╠═╡
run_nonlinear_policy_study(pocket_cube_curriculum_studies3.policy_studies.ac_nonlinear_study, 2f0 .^(-5:-3), 2f0 .^(-4:-3), [0f0, 0.1f0, 0.5f0, 0.7f0], [0f0, 0.1f0, 0.5f0, 0.7f0], 100_000; α_r̄ = 0.1f0, layer_size = 16, num_layers = 4)
  ╠═╡ =#

# ╔═╡ a57eb945-c7fb-427e-96c9-7c2c84ae50a8
begin
	...
	calculate_max_performance(min_scramble::Integer, max_scramble::Integer) = calculate_max_performance(min_scramble, max_scramble, 1)
end

# ╔═╡ c0b837da-8fc1-4f90-a9ae-14da66e3ae86
# begin
# 	const pocket_cube_nonlinear_value_base_setup = setup_value_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(2, 2), init_pocket_feature(), update_pocket_feature!)
# 	const pocket_cube_nonlinear_policy_base_setup = setup_policy_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(2, 2), init_pocket_feature(), update_pocket_feature!)
# 	const pocket_cube_nonlinear_value_parameters = pocket_cube_nonlinear_value_base_setup.parameters
# 	const pocket_cube_nonlinear_policy_params = pocket_cube_nonlinear_policy_base_setup.policy_params
# 	const pocket_cube_nonlinear_value_params = pocket_cube_nonlinear_policy_base_setup.value_params
	
# 	function setup_pocket_cube_nonlinear_value_training(min_scramble::Integer, max_scramble::Integer; use_gpu = false)
# 		feature_vector = use_gpu ? init_pocket_dense_feature() : init_pocket_feature()
# 		setup = setup_value_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble), feature_vector, update_pocket_feature!; fcann_parameters = pocket_cube_nonlinear_value_parameters)
# 	end

# 	function setup_pocket_cube_nonlinear_policy_training(min_scramble::Integer, max_scramble::Integer; use_gpu = false)
# 		feature_vector = use_gpu ? init_pocket_dense_feature() : init_pocket_feature()
# 		setup = setup_policy_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble), feature_vector, update_pocket_feature!; fcann_policy_parameters = pocket_cube_nonlinear_policy_params, fcann_value_parameters = pocket_cube_nonlinear_value_params)
# 	end
# end

# ╔═╡ f8b74af8-f864-49d3-8928-f43854a62984
#might wanna change this so the new setup function doesn't load from disk or save and then the save button only runs when pressed and creates a new setup with the save string.  also maybe add a training function that runs the train over multiple scramble passes and checks the performance as a sign of whether to move on or not and increase the trial time too

# ╔═╡ fa53e75c-d377-40b1-8f04-8e5a2c0ff397
#create a function that runs training for a given scramble move until the average reward matches perfect performance.  Start with 1000 steps and multiply by 10 until reaching 1 million before giving up

# ╔═╡ 9929ab32-4a44-47e1-9a6f-54c9b79a6ba4
#another idea is to try training at low scrambles which is easy but with high regularization and see if it leads to better results at higher scrambles faster

# ╔═╡ a4370fa3-ba54-4271-b9ef-eec0924c3d4c
BLAS.get_num_threads()

# ╔═╡ 2df0d9af-65fc-4ba6-9832-3ba41901c0d0
# [(n, l) for n in 2 .^(5:7) for l in 2 .^(1:3)] |> Map(t -> pocket_cube_nonlinear_value_train_curriculum(4f-2, 0.25f0, fill(t[1], t[2]), 1; use_gpu = false, ϵ = 0.001f0, α_r̄ = 0.1f0, l2 = 0.0f0)) |> tcollect

# ╔═╡ fee8c5e3-c530-4374-9f2a-189f061ef3ea
#add a function to calculate a score for all of the results by a weighted sum of each scramble with the percent of the time it is solved.  Then I can make a plot 2x2 with one axis being the number of layers, the other axis layer size, and the value being the score.

# ╔═╡ 8f8de807-df2e-4f74-b67e-76acdce9369a
# ╠═╡ disabled = true
#=╠═╡
const pocket_cube_nonlinear_policy_output = let
	use_gpu = true
	setup = setup_pocket_cube_nonlinear_policy_training(2, 2; use_gpu = use_gpu)
	output = setup.train_rate_decay(fill(256, 8), 1, 6f-3, 6f-3, 0.5f0, 0.5f0, 100_000; α_r̄ = 0.1f0, use_gpu = use_gpu, new_params = false)
	setup.save_params()
	output
end
  ╠═╡ =#

# ╔═╡ 06a7a1e5-8ca3-4dfa-9763-0dcd03b2024a
#=╠═╡
display_pocket_continuing_learning_output(pocket_cube_nonlinear_policy_output)
  ╠═╡ =#

# ╔═╡ 369f580c-be44-4627-821e-19b9bb8d9279
# ╠═╡ disabled = true
#=╠═╡
run_parameter_study(pocket_cube_curriculum_continuing_dp_λ_linear_parameter_study, [(3, α, α_r̄, λ) for α in 2f0 .^ (-11:-4) for α_r̄ in 2f0 .^ (-11:-8) for λ in (0f0, 0.1f0, 0.2f0, 0.3f0, 0.4f0, 0.5f0, 0.9f0, 0.95f0, 0.99f0)]; num_steps = 10_000)
  ╠═╡ =#

# ╔═╡ 6d7a013b-0f24-489c-99ae-c0d569997bab
# ╠═╡ disabled = true
#=╠═╡
run_parameter_study(pocket_cube_curriculum_continuing_actor_critic_linear_parameter_study, [(3, α_θ, α_w, α_r̄, λ_θ, λ_w) for α_θ in 2f0 .^ (-11:-4) for α_w in 2f0 .^ (-11:-4) for α_r̄ in 2f0 .^ (-13:-8) for λ_θ in (0f0, 0.1f0, 0.25f0, 0.5f0, 0.75f0, 0.9f0, 0.99f0) for λ_w in (0f0, 0.25f0, 0.5f0, 0.9f0, 0.99f0)]; num_steps = 10_000)
  ╠═╡ =#

# ╔═╡ a4c6f112-1e4d-452c-a6ec-0c2e8944fe7d


# ╔═╡ 33aca7bc-3631-4f69-94f1-3b6407bd7914
#next step is to do parameter studies for both of these methods with different scramble rates, then repeat that with non-linear approximation

# ╔═╡ 3e36bfae-8d6b-443d-8288-71d22f41bd05
function init_pocket_feature()
	v = BinaryFeatureVector(24*24)
	v.num_features = 24
	v.active_features = collect(1:24)
	return v
end

# ╔═╡ b9b6ad8d-60c1-4f3a-ae6a-6988d8e78903
init_pocket_dense_feature() = zeros(Float32, 24*24)

# ╔═╡ a63e8895-390d-43d6-88f0-6bda6318b5ef
function update_pocket_feature!(v::BinaryFeatureVector, s::AbstractVector{I}) where I<:Integer
	for i in 1:24
		f = s[i]
		base_ind = 24*(i-1)
		v.active_features[i] = base_ind + f
	end
	return v
end

# ╔═╡ 7cc6dbf3-fc92-4e0b-8e62-9c2cfe8fbde4
function update_pocket_feature!(v::Vector{T}, s::AbstractVector{I}) where {I<:Integer, T<:Real}
	v .= zero(T)
	for i in 1:24
		f = s[i]
		base_ind = 24*(i-1)
...
# ╔═╡ 651bfb76-afa3-45a3-a6c8-1d434072ba22
update_pocket_feature!(v, s::NamedTuple) = update_pocket_feature!(v, s.cube)

# ╔═╡ 146192b4-e2c5-436c-8d88-594384d086aa
#=╠═╡
function create_continuing_trial(f::Function)
	run_trial(args...; kwargs...) = mean(f(args...; kwargs...).reward_history)
end
  ╠═╡ =#

# ╔═╡ 56d0d420-37d7-411f-8c07-43f6ca296718
#=╠═╡
function run_parameter_study(study::NamedTuple, args::Vector; kwargs...)
	for a in args
		study.update_results!(a...; kwargs...)
	end
	output = [(;k..., value = study.results[k]) for k in keys(study.results)] |> DataFrame
	sort(output, :value; rev = true)
end
  ╠═╡ =#

# ╔═╡ 3b47c645-e9e3-4f58-ba98-5ba116ac932c
const actor_critic_base_params = [(α_θ, α_w, α_r̄, λ_θ, λ_w) for α_θ in 2f0 .^(-10:-5) for α_w in 2f0 .^(-10:-5) for α_r̄ in 2f0 .^(-9:-7) for λ_θ in [0f0, 0.1f0, 0.5f0, 0.9f0] for λ_w in [0f0, 0.1f0, 0.5f0, 0.9f0]]

# ╔═╡ 6683e237-e96a-47ae-bf63-2d51b25637bb
const dp_λ_base_params = [(α, α_r̄, λ) for α in 2f0 .^(-10:-5) for α_r̄ in 2f0 .^(-9:-7) for λ in [0f0, 0.1f0, 0.5f0, 0.9f0]]

# ╔═╡ e77bc037-c673-49c6-a1ba-121d226e06be
# ╠═╡ disabled = true
#=╠═╡
run_parameter_study(pocket_cube_continuing_actor_critic_linear_parameter_study, actor_critic_base_params; num_steps = 1_000)
  ╠═╡ =#

# ╔═╡ f085423a-e63b-45c5-906d-2a7451c91bfc
const actor_critic_fcann_base_params = [(α_θ, α_w, α_r̄, λ_θ, λ_w) for α_θ in 2f0 .^(-10:-8) for α_w in 2f0 .^(-10:-8) for α_r̄ in 2f0 .^(-9:-8) for λ_θ in [0f0, 0.5f0] for λ_w in [0f0, 0.5f0]]

# ╔═╡ f7610b14-a9de-4f40-ae2e-eea5511da103
# ╠═╡ disabled = true
#=╠═╡
run_parameter_study(pocket_cube_continuing_actor_critic_fcann_parameter_study, actor_critic_fcann_base_params; num_layers = 2, layer_size = 32, num_steps = 100_000)
  ╠═╡ =#

# ╔═╡ cd369692-574d-4678-b523-4cf835524b90
const pocket_cube_fcann_test_hidden = fill(64, 2)

# ╔═╡ 7033cc56-4fc5-4504-bc74-749e3059ffe5
md"""
### Reward Process Based on Tabular Solution
"""

# ╔═╡ 0d88866b-b04d-415f-854e-78a7ae6d6c1c
md"""
#### Linear Approximaiton of Value Function
"""

# ╔═╡ a8a6088f-f7ea-4536-a454-8ab017e27a4c
md"""
#### Non-linear Approximation of Value Function
"""

# ╔═╡ 644c6ecd-11b5-48be-809b-f2392f8c6afc
const td_λ_fcann_episodic_mrp_mastery_results = Dict{NamedTuple, NamedTuple}()

# ╔═╡ 439a346e-ca98-4dac-8cb0-1f6d66567700
#in order to apply gpu approximation to this, need to make a dense version of the feature vector update to pass to the GPU

# ╔═╡ 0c0d6dc2-5a7d-4e9c-93df-0cb54bdef5d9
...
# ╔═╡ 514abe7b-0625-4eae-9dcd-d198924b7508
const td_λ_episodic_mrp_nonlinear_layers = fill(128, 4)

# ╔═╡ 8de307a8-5fb5-43d8-8504-a0c449541d10
# ╠═╡ disabled = true
#=╠═╡
for layer_size = 2 .^ (6:8)
	for num_layers = 2 .^ (1:2)
		layers = fill(layer_size, num_layers)
		test = initialize_pocket_episodic_mrp_nonlinear_test(layers)
		test.update!(1_000_000, 0.5f0, 1f-3; min_scramble = 11, max_scramble = 11)
	end
end
  ╠═╡ =#

# ╔═╡ dbad3db0-7e54-4c2d-917a-38610a4c1624
#=╠═╡
@bind save_params CounterButton("Save Parameters to Disk")
  ╠═╡ =#

# ╔═╡ 891d4c31-218d-44f9-a9d8-bf609ac82f8a
save_fcann_params(parameters::FCANNParams, name::String) = FCANN.writeParams([parameters.weights], name)

# ╔═╡ cd390f26-443c-45c1-9d2a-e8ade044a93c
function save_episodic_pocket_mrp_fcann_params()
	for k in keys(td_λ_fcann_episodic_mrp_mastery_results)
		params = td_λ_fcann_episodic_mrp_mastery_results[k].params
		name = "pocket_mrp_td_λ_fcann_params_$(string(k.layers))_hidden_$(params.reslayers)_reslayer.bin"
		save_fcann_params(params, name)
	end
end

# ╔═╡ 04383bbf-cacb-4938-a605-8b6b4e903899
#=╠═╡
if save_params > 0
	save_episodic_pocket_mrp_fcann_params()
end
  ╠═╡ =#

# ╔═╡ 2d20c73a-d580-40a9-a56a-e2c8ceb3f00d
begin
	calculate_num_params(p::FCANNParams) = p |> NonTabularRL.get_network_dimensions |> p -> FCANN.getNumParams(p...)
	calculate_num_params(w::Array) = length(w)
end
...
# ╔═╡ 19fddbf2-db03-4d1b-86de-e4cda5984c94
const pocket_cube_fcann_test_params = initialize_fcann_params(init_pocket_feature(), pocket_cube_fcann_test_hidden, length(all_pocket_moves), 1, true)
...
# ╠═╡ disabled = true
#=╠═╡
@benchmark solve_pocket_cube_ida_star(rand(pocket_mdp.states); show_message = false)
  ╠═╡ =#
...
# ╠═╡ disabled = true
#=╠═╡
const mrp_nonlinear_layers = fill(256, 30)
  ╠═╡ =#
...
#=╠═╡
const dp_λ_2step_result = run_dp_λ_rubiks_nstep_linear_test(0.9f0, 0.95f0, 2, 2, 5; num_steps = 50_000, α = 1f-2, ϵ = 0.01f0, trace_type = NonTabularRL.ReplacingTrace())
  ╠═╡ =#
...
@use_effect([]) do
	@spawn begin
		...
		result = run_dp_α_decay_step_mastery_nmove_fcann_test!(dp_mastery_2move_fcann_params, 6, 8, 2, deepcopy(rubiks_binary_feature), update_rubiks_feature!, 1_000_000, 1f-3; ϵ = 0.01f0)
...
@use_effect([]) do
	@spawn begin
		...
		result = run_dp_α_decay_step_mastery_nmove_fcann_test!(dp_mastery_2move_fcann_params2, 7, 9, 2, deepcopy(rubiks_binary_feature), update_rubiks_feature!, 1_000_000, 1f-4; ϵ = 0.01f0)
...
# ╔═╡ fd117080-ac4f-4bcf-b900-eee7cbef3da2
365*length(pocket_states) * 1e-3 / 60 / 60
...
const pocket_cube_solution = value_iteration_v(pocket_mdp, 1f0; usethreads=true, show_message = false, make_final_policy = TabularRL.make_greedy_bit_policy)

# ╔═╡ 8b8bcacd-0d00-4c72-86b4-e3bdf43d653f
extrema(pocket_cube_solution.final_value)

# ╔═╡ b384cf65-0865-4b54-81af-0ad9fbd3db97
const test_episode = runepisode(pocket_mdp; π = pocket_cube_solution.optimal_policy)

# ╔═╡ 2f841a1d-510d-4fee-9631-6af861490e96
const pocket_state_mdp = StateMDP(pocket_mdp)

# ╔═╡ 4cbc56b2-85ac-47bb-80bf-963154cf3f48
const test_pocket_cube = rand(pocket_mdp.states)

# ╔═╡ 833074cf-5035-4f12-96c1-9f919ff39f60
begin
	function rotate_pocket_cube2(s::SVector{24, UInt8}, m::Tuple{Int64, Int64})
		s′ = rotate_pocket_cube(s, m[1])
		for _ in 2:m[2]
			s′ = rotate_pocket_cube(s′, m[1])
		end
		return s′
	...
	rotate_pocket_cube2(s::SVector{24, UInt8}, i_a::Integer) = rotate_pocket_cube2(s, all_pocket_moves[i_a])
end

# ╔═╡ 5a260743-84eb-487e-bfe6-c45bc10b4dce
function initialize_pocket_cube(num_moves::Integer)
	s = solved_pocket_cube
	a = rand(all_pocket_moves)
	s′ = rotate_pocket_cube2(s, a)
	for _ in 2:num_moves
		a = rand(get_next_pocket_moves(a))
		s′ = rotate_pocket_cube2(s′, a)
	end
	return s′
end

# ╔═╡ 12b0da2f-6c92-428a-b501-fe42e8e71e25
function pocket_state_dense_step(s, i_a)
	(s == solved_pocket_cube) && return (0f0, initialize_pocket_cube(11))
	score1 = score_pocket_cube(s)
	s′ = rotate_pocket_cube2(s, i_a)
	(s′ == solved_pocket_cube) && return (1f0 - score1, initialize_pocket_cube(11))
	score2 = score_pocket_cube(s′)
	r = score2 - score1
	return (r, s′)
end

# ╔═╡ d66fbdd6-bfba-4f9a-ac41-f3d748544b39
function pocket_state_sparse_step(s, i_a; scramble_moves::Integer = 11)
	(s == solved_pocket_cube) && return (0f0, initialize_pocket_cube(scramble_moves))
	s′ = rotate_pocket_cube2(s, i_a)
	(s′ == solved_pocket_cube) && return (1f0, initialize_pocket_cube(scramble_moves))
	return (0f0, s′)
end

# ╔═╡ a05b196f-c053-45e9-9345-b58496cd5bb8
const pocket_state_continuing_mdp = begin
	StateMDP(all_pocket_moves, StateMDPTransitionDeterministic(pocket_state_dense_step, solved_pocket_cube), () -> initialize_pocket_cube(11), Returns(false))
end	

# ╔═╡ dd2b3afb-a83e-42ea-a032-4bf5530bdae0
#=╠═╡
begin
	function pocket_cube_continuing_actor_critic_linear(α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, kwargs...)
		actor_critic_with_eligibility_traces_linear(pocket_state_continuing_mdp, λ_θ, λ_w, num_steps, init_pocket_feature(), update_pocket_feature!; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
	end

	const pocket_cube_continuing_actor_critic_linear_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_continuing_actor_critic_linear), (:α_θ, :α_w, :α_r̄, :λ_θ, :λ_w), (num_steps = 10_000, trace_type = AccumulatingTrace()))
end
  ╠═╡ =#

# ╔═╡ 640eed54-9cc7-4559-bfbb-5ad4625c2bb2
#=╠═╡
const pocket_cube_linear_actor_critic_test = pocket_cube_continuing_actor_critic_linear(1f-3, 1f-3, 1f-2, 0.5f0, 0.5f0; num_steps = 100_000)
  ╠═╡ =#

# ╔═╡ 56966065-6b89-4631-811b-0940a3519dfb
#=╠═╡
plot_rewards(pocket_cube_linear_actor_critic_test.reward_history, 1000, 1000)
  ╠═╡ =#

# ╔═╡ 4ed0fbea-59ef-41f2-84bb-65f291b2d17c
#=╠═╡
begin
	function pocket_cube_continuing_actor_critic_fcann(α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, num_layers = 2, layer_size = 4, kwargs...)
		hidden_layers = fill(layer_size, num_layers)
		actor_critic_with_eligibility_traces_fcann(pocket_state_continuing_mdp, λ_θ, λ_w, num_steps, init_pocket_feature(), update_pocket_feature!, hidden_layers; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
	end

	const pocket_cube_continuing_actor_critic_fcann_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_continuing_actor_critic_fcann), (:α_θ, :α_w, :α_r̄, :λ_θ, :λ_w), (num_steps = 10_000, trace_type = AccumulatingTrace(), reslayers = 1, num_layers = 2, layer_size = 4))
end
  ╠═╡ =#

# ╔═╡ 3a721740-1cc7-45c0-98e1-4e78e940cd9d
const pocket_cube_fcann_actor_critic_test = actor_critic_with_eligibility_traces_fcann(pocket_state_continuing_mdp, 0.9f0, 0.1f0, 10_000, init_pocket_feature(), update_pocket_feature!, pocket_cube_fcann_test_hidden; α_θ = 5f-2, α_w = 5f-2, α_r̄ = 0.01f0, policy_params = pocket_cube_fcann_test_params)

# ╔═╡ 1267acb8-a5c3-4614-a2e3-f9ae32f04a0c
#=╠═╡
plot_rewards(pocket_cube_fcann_actor_critic_test.reward_history, 1000, 1000)
  ╠═╡ =#

# ╔═╡ 30d2d441-d91c-4b5e-9a7b-aa33694c3c92
pocket_cube_fcann_actor_critic_test.policy_function(rand(pocket_mdp.states))

# ╔═╡ a83d4479-0d4d-421c-935b-924126e16a2f
pocket_cube_fcann_actor_critic_test.value_function(rand(pocket_mdp.states))

# ╔═╡ 203bafc0-060b-4856-8b10-a2402a42124f
const pocket_state_sparse_continuing_mdp = begin
	StateMDP(all_pocket_moves, StateMDPTransitionDeterministic(pocket_state_sparse_step, solved_pocket_cube), () -> initialize_pocket_cube(11), Returns(false))
end	

# ╔═╡ 1b72205a-c860-462a-a47c-9c906685936e
#=╠═╡
begin
	function pocket_cube_sparse_continuing_actor_critic_linear(α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, kwargs...)
		actor_critic_with_eligibility_traces_linear(pocket_state_sparse_continuing_mdp, λ_θ, λ_w, num_steps, init_pocket_feature(), update_pocket_feature!; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
	end

	const pocket_cube_sparse_continuing_actor_critic_linear_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_sparse_continuing_actor_critic_linear), (:α_θ, :α_w, :α_r̄, :λ_θ, :λ_w), (num_steps = 10_000, trace_type = AccumulatingTrace()))
end
  ╠═╡ =#

# ╔═╡ f4acc6cf-8306-4912-a732-7f45596498d6
#=╠═╡
const pocket_cube_sparse_linear_actor_critic_test = pocket_cube_sparse_continuing_actor_critic_linear(1f-2, 1f-2, 1f-2, 0.1f0, 0.9f0; num_steps = 100_000, trace_type = AccumulatingTrace())
  ╠═╡ =#

# ╔═╡ 581aba58-3379-46d0-9096-dcf98226feaa
#=╠═╡
plot_rewards(pocket_cube_sparse_linear_actor_critic_test.average_reward_history, 10, 1000)
  ╠═╡ =#

# ╔═╡ 7e29c2f8-9e78-4f59-9a9f-1ec1d586f7d3
#=╠═╡
pocket_cube_sparse_linear_actor_critic_test.policy_function(rand(pocket_mdp.states))
  ╠═╡ =#

# ╔═╡ 06604e3e-833d-4b7a-8c31-7501281bb93a
#=╠═╡
pocket_cube_sparse_linear_actor_critic_test.value_function(rand(pocket_mdp.states))
  ╠═╡ =#

# ╔═╡ 6a123216-fe9a-4fd3-9e11-5dc3bdd7ab5e
function pocket_curriculum_sparse_continuing_step(s::@NamedTuple{cube::SVector{24, UInt8}, scramble_moves::Int64, move_count::Int64}, i_a; min_moves::Integer = 11, max_moves::Integer = 11)
	scramble_moves = rand(min_moves:max_moves)
	(s.cube == solved_pocket_cube) && return (0f0, (cube = initialize_pocket_cube(scramble_moves), scramble_moves = scramble_moves, move_count = 0))
	(s.move_count > 2*s.scramble_moves) && return (0f0, (cube = initialize_pocket_cube(scramble_moves), scramble_moves = scramble_moves, move_count = 0))
	(s.move_count > 22) && return (0f0, (cube = initialize_pocket_cube(scramble_moves), scramble_moves = scramble_moves, move_count = 0))
	cube′ = rotate_pocket_cube2(s.cube, i_a)
	(cube′ == solved_pocket_cube) && return (1f0, (cube = initialize_pocket_cube(scramble_moves), scramble_moves = scramble_moves, move_count = 0))
	return (0f0, (cube = cube′, scramble_moves = s.scramble_moves, move_count = s.move_count + 1))
end

# ╔═╡ 562f081d-df5c-4461-a3c2-6508dbf0eddd
function create_curriculum_pocket_continuing_mdp(min_moves::Integer, max_moves::Integer)
	function init_state() 
		scramble_moves = rand(min_moves:max_moves)
		(cube = initialize_pocket_cube(scramble_moves), scramble_moves = scramble_moves, move_count = 0)
	end
	step(s, i_a) = pocket_curriculum_sparse_continuing_step(s, i_a; min_moves = min_moves, max_moves = max_moves)
	StateMDP(all_pocket_moves, StateMDPTransitionDeterministic(step, init_state()), init_state, Returns(false))
end

# ╔═╡ 9a8a80b7-af40-4abc-9b0e-dbb56de6bbd2
function create_pocket_cube_curriculum_parameter_studies(min_scramble, max_scramble)
	mdp = create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble)
	value_studies = setup_continuing_value_parameter_studies(mdp, init_pocket_feature(), update_pocket_feature!, true)
	policy_studies = setup_continuing_policy_parameter_studies(mdp, init_pocket_feature(), update_pocket_feature!)
	return (value_studies = value_studies, policy_studies = policy_studies)
end

# ╔═╡ bdcb1154-620d-4d74-96be-0dceda8c129d
const pocket_cube_curriculum_studies1 = create_pocket_cube_curriculum_parameter_studies(2, 2)

# ╔═╡ 06ce78b2-e76a-47b5-be08-78b1ca1edca9
const pocket_cube_curriculum_studies2 = create_pocket_cube_curriculum_parameter_studies(3, 3)

# ╔═╡ 4d8fc956-1e6f-4e14-be34-961625aed20e
const pocket_cube_curriculum_studies3 = create_pocket_cube_curriculum_parameter_studies(2, 5)

# ╔═╡ 20be0ad9-1ff7-4ead-9339-414e78efa3c8
begin
	const pocket_cube_linear_value_base_setup = setup_value_linear_training("pocket_cube_curriculum_linear", false, create_curriculum_pocket_continuing_mdp(2, 2), init_pocket_feature(), update_pocket_feature!)
	const pocket_cube_linear_value_dp_params = pocket_cube_linear_value_base_setup.dp_params
	
	function setup_pocket_cube_linear_value_training(min_scramble::Integer, max_scramble::Integer)
		setup = setup_value_linear_training("pocket_cube_curriculum_linear", false, create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble), init_pocket_feature(), update_pocket_feature!; linear_dp_params = pocket_cube_linear_value_dp_params)
	end
end

# ╔═╡ a7d3c63a-b48f-4632-8422-0a557f24c834
const pocket_cube_linear_value_setup = setup_pocket_cube_linear_value_training(3, 4)

# ╔═╡ 0a246afc-01d6-4eab-bf7a-fafa23c0bfd1
const pocket_cube_linear_value_output = pocket_cube_linear_value_setup.train_rate_decay(1f-3, 0.7f0, 100_000; ϵ = 0.001f0, α_r̄ = 0.1f0, use_dp = true)

# ╔═╡ f72ab3b8-b88e-4589-b6a5-d0bfbde494e6
begin
	const pocket_cube_linear_policy_base_setup = setup_policy_linear_training("pocket_cube_curriculum_linear", false, create_curriculum_pocket_continuing_mdp(2, 2), init_pocket_feature(), update_pocket_feature!)
	
	const pocket_cube_linear_policy_params = pocket_cube_linear_policy_base_setup.policy_params
	const pocket_cube_linear_value_params = pocket_cube_linear_policy_base_setup.value_params
	function setup_pocket_cube_linear_policy_training(min_scramble::Integer, max_scramble::Integer)
		setup = setup_policy_linear_training("pocket_cube_curriculum_linear", false, create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble), init_pocket_feature(), update_pocket_feature!; linear_policy_params = pocket_cube_linear_policy_params, linear_value_params = pocket_cube_linear_value_params)
	end
end

# ╔═╡ 1f59da08-3a04-43b8-b35b-17ec96878a95
const pocket_cube_linear_policy_setup = setup_pocket_cube_linear_policy_training(2, 5)

# ╔═╡ b43566d3-e267-4b52-9933-346fca0e828d
const pocket_cube_linear_policy_output = pocket_cube_linear_policy_setup.train_rate_decay(1f-2, 1f-3, 0.5f0, 0.1f0, 100_000; α_r̄ = 0.1f0)

# ╔═╡ d7a61f17-b772-4a50-b8a3-00808ad087af
begin
	function pocket_cube_nonlinear_value_training(hidden_layers::Vector{Int64}, reslayers::Integer, training_steps::Integer; use_gpu = false, show_message = false)
		feature_vector = use_gpu ? init_pocket_dense_feature() : init_pocket_feature()
		setup = setup_value_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(2, 2), feature_vector, update_pocket_feature!; show_message = show_message)
		setup.train(hidden_layers, reslayers, 0f0, 0f0, training_steps; use_dp = true, use_gpu = use_gpu, new_params = false)
	end

	function pocket_cube_nonlinear_policy_training(hidden_layers::Vector{Int64}, reslayers::Integer, training_steps::Integer; use_gpu = false, show_message = false)
		feature_vector = use_gpu ? init_pocket_dense_feature() : init_pocket_feature()
		setup = setup_policy_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(2, 2), feature_vector, update_pocket_feature!; show_message = show_message)
		setup.train(hidden_layers, reslayers, 0f0, 0f0, 0f0, 0f0, training_steps; use_gpu = use_gpu, new_params = false)
	end
		
	function pocket_cube_nonlinear_value_training(α::Float32, λ::Float32, hidden_layers::Vector{Int64}, reslayers::Integer, min_scramble::Integer, max_scramble::Integer, training_steps::Integer; use_gpu = false, show_message = false, kwargs...)
		feature_vector = use_gpu ? init_pocket_dense_feature() : init_pocket_feature()
		setup = setup_value_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble), feature_vector, update_pocket_feature!; show_message = show_message)
	...
	function pocket_cube_nonlinear_policy_training(α_θ::Float32, α_w::Float32, λ_θ::Float32, λ_w::Float32, hidden_layers::Vector{Int64}, reslayers::Integer, min_scramble::Integer, max_scramble::Integer, training_steps::Integer; use_gpu = false, show_message = false, kwargs...)
		feature_vector = use_gpu ? init_pocket_dense_feature() : init_pocket_feature()
		setup = setup_policy_nonlinear_training("pocket_cube_curriculum_fcann", false, create_curriculum_pocket_continuing_mdp(min_scramble, max_scramble), feature_vector, update_pocket_feature!; show_message = show_message)
...
# ╔═╡ d875b321-a1b3-40f3-ad68-c137e85e471e
function pocket_cube_nonlinear_value_train_scramble(α::Float32, λ::Float32, hidden_layers::Vector{Int64}, reslayers::Integer, min_scramble::Integer, max_scramble::Integer; init_steps::Integer = 1000, step_multiplier::Integer = 2, max_steps::Integer = 1_000_000, kwargs...)
	performance_target = calculate_max_performance(min_scramble, max_scramble)
	
	@info "Starting training with scramble moves: $((min_scramble, max_scramble)) and $init_steps steps.  Performance target is $performance_target"
	output = pocket_cube_nonlinear_value_training(α, λ, hidden_layers, reslayers, min_scramble, max_scramble, init_steps; kwargs...)
	performance = output.performance
	@info "Completed training with scramble moves: $((min_scramble, max_scramble)) and $init_steps steps.  Performance is $performance vs target of $performance_target"

	if (performance ≥ 0.99f0*performance_target) 
		@info "Successfully reached target performance of $performance_target using $init_steps steps"
		return (;output..., hit_target = true, performance_target = performance_target)
	end
	
	training_steps = init_steps
	while (performance < 0.99f0*performance_target) && (training_steps ≤ max_steps)
		training_steps *= step_multiplier
		@info "Starting training with scramble moves: $((min_scramble, max_scramble)) and $training_steps steps.  Performance target is $performance_target"
		output = pocket_cube_nonlinear_value_training(α, λ, hidden_layers, reslayers, min_scramble, max_scramble, training_steps; kwargs...)
		performance = output.performance
		@info "Completed training with scramble moves: $((min_scramble, max_scramble)) and $training_steps steps.  Performance is $performance vs target of $performance_target"
	end

	hit_target = (performance ≥ 0.99f0*performance_target)

	if training_steps > max_steps
		@info "Finished training with maximum allowed steps of $max_steps"
	else
		@info "Successfully reached target performance of $performance_target using $training_steps steps"
	end
	return (;output..., hit_target = hit_target, performance_target = performance_target)
end

# ╔═╡ bc3362d1-140e-48a6-bc6b-c557b6e73d1e
#=╠═╡
begin
	function pocket_cube_curriculum_continuing_actor_critic_linear(scramble_moves::Integer, α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, kwargs...)
		mdp = create_curriculum_pocket_continuing_mdp(scramble_moves)
		actor_critic_with_eligibility_traces_linear(mdp, λ_θ, λ_w, num_steps, init_pocket_feature(), update_pocket_feature!; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
	end

	function setup_pocket_cube_curriculum_continuing_actor_critic_linear()
		policy_params = initialize_linear_parameters(24*24, length(all_pocket_moves), 0f0)
		value_params = initialize_linear_parameters(24*24, 0f0)
		update_parameters!(scramble_moves, α_θ, α_w, α_r̄, λ_θ, λ_w; kwargs...) = pocket_cube_curriculum_continuing_actor_critic_linear(scramble_moves, α_θ, α_w, α_r̄, λ_θ, λ_w; kwargs..., policy_params = policy_params, value_params = value_params)
		return (policy_parameters = policy_params, value_parameters = value_params, update_parameters! = update_parameters!)
	end

	function pocket_cube_curriculum_continuing_dp_λ_linear(scramble_moves::Integer, α, α_r̄, λ; num_steps = 10_000, kwargs...)
		mdp = create_curriculum_pocket_continuing_mdp(scramble_moves)
		dp_λ_linear(mdp, λ, num_steps, init_pocket_feature(), update_pocket_feature!; α = α, α_r̄ = α_r̄, kwargs...)
	end

	function setup_pocket_cube_curriculum_continuing_dp_λ_linear()
		parameters = initialize_linear_parameters(24*24, 0f0)
		update_parameters!(scramble_moves, α, α_r̄, λ; kwargs...) = pocket_cube_curriculum_continuing_dp_λ_linear(scramble_moves, α, α_r̄, λ; kwargs..., parameters = parameters)
		return (parameters = parameters, update_parameters! = update_parameters!)
	end
		

	const pocket_cube_curriculum_continuing_actor_critic_linear_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_curriculum_continuing_actor_critic_linear), (:scramble_moves, :α_θ, :α_w, :α_r̄, :λ_θ, :λ_w), (num_steps = 10_000, trace_type = AccumulatingTrace()))

	const pocket_cube_curriculum_continuing_dp_λ_linear_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_curriculum_continuing_dp_λ_linear), (:scramble_moves, :α, :α_r̄, :λ), (num_steps = 10_000, trace_type = AccumulatingTrace()))
end
  ╠═╡ =#

# ╔═╡ e8bee8d2-57c3-46cf-b08c-18ba0aba2079
#=╠═╡
const pocket_cube_curriculum_continuing_dp_λ_linear_test = setup_pocket_cube_curriculum_continuing_dp_λ_linear()
  ╠═╡ =#

# ╔═╡ c161a866-4f40-43cd-beaa-118cff14cc32
#=╠═╡
const pocket_cube_curriculum_continuing_actor_critic_linear_test = setup_pocket_cube_curriculum_continuing_actor_critic_linear()
  ╠═╡ =#

# ╔═╡ 7f802215-4a62-4764-b972-ff9539a8d2c9
#=╠═╡
begin
	function pocket_cube_curriculum_continuing_actor_critic_fcann(scramble_moves::Integer, hidden_layers, α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, kwargs...)
		mdp = create_curriculum_pocket_continuing_mdp(scramble_moves)
		actor_critic_with_eligibility_traces_fcann(mdp, λ_θ, λ_w, num_steps, init_pocket_feature(), update_pocket_feature!, hidden_layers; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
	end

	function setup_pocket_cube_curriculum_continuing_actor_critic_fcann(hidden_layers, reslayers)
		policy_params = initialize_fcann_params(24*24, hidden_layers, length(all_pocket_moves), reslayers, true)
		value_params = NonTabularRL.initialize_fcann_value_params(policy_params, true)
		update_parameters!(scramble_moves, α_θ, α_w, α_r̄, λ_θ, λ_w; kwargs...) = pocket_cube_curriculum_continuing_actor_critic_fcann(scramble_moves, hidden_layers, α_θ, α_w, α_r̄, λ_θ, λ_w; kwargs..., policy_params = policy_params, value_params = value_params, reslayers = reslayers)
		return (policy_parameters = policy_params, value_parameters = value_params, update_parameters! = update_parameters!)
	end

	function pocket_cube_curriculum_continuing_dp_λ_fcann(scramble_moves::Integer, hidden_layers, α, α_r̄, λ; num_steps = 10_000, kwargs...)
		mdp = create_curriculum_pocket_continuing_mdp(scramble_moves)
		dp_λ_fcann(mdp, λ, num_steps, init_pocket_feature(), update_pocket_feature!, hidden_layers; α = α, α_r̄ = α_r̄, kwargs...)
	end

	function setup_pocket_cube_curriculum_continuing_dp_λ_fcann(hidden_layers, reslayers)
		parameters = initialize_fcann_params(24*24, hidden_layers, 1, reslayers, true)
		update_parameters!(scramble_moves, α, α_r̄, λ; kwargs...) = pocket_cube_curriculum_continuing_dp_λ_fcann(scramble_moves, hidden_layers, α, α_r̄, λ; kwargs..., parameters = parameters, reslayers = reslayers)
		return (parameters = parameters, update_parameters! = update_parameters!)
	end
		

	const pocket_cube_curriculum_continuing_actor_critic_fcann_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_curriculum_continuing_actor_critic_fcann), (:scramble_moves, :hidden_layers, :α_θ, :α_w, :α_r̄, :λ_θ, :λ_w), (num_steps = 10_000, reslayers = 1, trace_type = AccumulatingTrace()))

	const pocket_cube_curriculum_continuing_dp_λ_fcann_parameter_study = setup_parameter_study(create_continuing_trial(pocket_cube_curriculum_continuing_dp_λ_fcann), (:scramble_moves, :hidden_layers, :α, :α_r̄, :λ), (num_steps = 10_000, trace_type = AccumulatingTrace()))
end
  ╠═╡ =#

# ╔═╡ c7a97bea-ceeb-4cf5-91b6-5cc5d0de7c38
#=╠═╡
const pocket_cube_curriculum_continuing_dp_λ_fcann_test = setup_pocket_cube_curriculum_continuing_dp_λ_fcann(fill(64, 2), 1)
  ╠═╡ =#

# ╔═╡ ca7479b5-543b-41ad-90e4-c81b8950b3d3
#=╠═╡
const pocket_cube_curriculum_continuing_actor_critic_fcann_test = setup_pocket_cube_curriculum_continuing_actor_critic_fcann(fill(64, 4), 1)
  ╠═╡ =#

# ╔═╡ 4d5778c4-e82b-48b3-a2f2-c37737e2b6a3
#=╠═╡
const pocket_actor_critic_fcann_results = pocket_cube_curriculum_continuing_actor_critic_fcann_test.update_parameters!(5, 1f-2, 2f-4, 0.0002f0, 0.75f0, 0.25f0; num_steps = 100_000) 
  ╠═╡ =#

# ╔═╡ 1808a482-6cdb-43a5-a9e9-5f853273a480
#=╠═╡
pocket_actor_critic_fcann_results.policy_function((cube = initialize_pocket_cube(2), scramble_moves = 2, move_count = 0))
  ╠═╡ =#

# ╔═╡ 989e91f7-e331-4687-bf8c-1efbca516997
#=╠═╡
pocket_actor_critic_fcann_results.value_function((cube = initialize_pocket_cube(2), scramble_moves = 2, move_count = 0))
  ╠═╡ =#

# ╔═╡ bdb84292-c494-41a7-8a61-4cd14d8fc5f1
function pocket_curriculum_sparse_episodic_step(s::@NamedTuple{cube::SVector{24, UInt8}, scramble_moves::Int64, move_count::Int64}, i_a)
	cube′ = rotate_pocket_cube2(s.cube, i_a)
	r = Float32(cube′ == solved_pocket_cube)
	return (r, (cube = cube′, scramble_moves = s.scramble_moves, move_count = s.move_count + 1))
end

# ╔═╡ 392bdf01-31ac-4814-ba20-6c24b6396b3d
function create_curriculum_pocket_episodic_mdp(min_moves::Integer, max_moves::Integer)
	
	function init_state() 
		scramble_moves = rand(min_moves:max_moves)
		(cube = initialize_pocket_cube(scramble_moves), scramble_moves = scramble_moves, move_count = 0)
	end
	step(s, i_a) = pocket_curriculum_sparse_episodic_step(s, i_a)
	function isterm(s)
		(s.cube == solved_pocket_cube) ||
		(s.move_count > 2*s.scramble_moves) ||
		s.move_count > 22
	end
	StateMDP(all_pocket_moves, StateMDPTransitionDeterministic(step, init_state()), init_state, isterm)
end

# ╔═╡ 345acba4-1881-4369-9bdc-1cb5040678fc
function get_pocket_scramble_statistic(scramble::Integer, π::Function, ntrials::Integer)
	mdp = create_curriculum_pocket_episodic_mdp(scramble, scramble)
	1:ntrials |> Map(i -> runepisode(mdp; π = π) |> out -> out[1][1].cube == solved_pocket_cube ? 1f0 : out[3][end]) |> foldxt(+) |> x -> x / ntrials
end

# ╔═╡ 7cccfc40-e8d6-4701-adc8-1106cc3974d8
function get_pocket_policy_scramble_statistic(scramble::Integer, output::NamedTuple, ntrials::Integer)
	get_pocket_scramble_statistic(scramble, output.policy_sample_action, ntrials)
end

# ╔═╡ a4195e09-7435-424b-9c94-dbcab4d7e40d
function get_pocket_policy_scramble_statistics(output, min_scramble::Integer, max_scramble::Integer; ntrials = 1000) 
	[(n = n, solve_rate = get_pocket_policy_scramble_statistic(n, output, ntrials)) for n in min_scramble:max_scramble]
end

# ╔═╡ 6b286bc8-dc5c-4bb6-8495-35788add0281
function get_pocket_value_scramble_statistic(scramble::Integer, output::NamedTuple, ntrials::Integer)
	π(s) = output.value_function(s; output.form_kwargs()...).maximizing_action
	get_pocket_scramble_statistic(scramble, π, ntrials)
end

# ╔═╡ d62b940e-64ec-4fdb-bcfd-770f260159cc
function get_pocket_value_scramble_statistics(output, min_scramble::Integer, max_scramble::Integer; ntrials = 1000) 
	[(n = n, solve_rate = get_pocket_value_scramble_statistic(n, output, ntrials)) for n in min_scramble:max_scramble]
end

# ╔═╡ 707a2df2-4cb6-4043-bef9-aec86609b0c3
#=╠═╡
function display_pocket_continuing_learning_output(output::NamedTuple; min_scramble = 1, max_scramble = 11)
	reward_plot = output.reward_history |> v -> plot_rewards(v, min(100, length(v)), min(1000, length(v)))
	f_stats = haskey(output, :policy_sample_action) ? get_pocket_policy_scramble_statistics : get_pocket_value_scramble_statistics
	stats = f_stats(output, min_scramble, max_scramble)
	@htl("""
	<div style = "display: flex;">
	<div style = "width: 75%">
	$reward_plot
...
# ╔═╡ ac010889-58f7-4af8-87a9-deed7c70426d
#=╠═╡
display_pocket_continuing_learning_output(pocket_cube_linear_value_output)
  ╠═╡ =#

# ╔═╡ 5b7ed599-e51a-49ec-abfa-70c60bb168b3
#=╠═╡
display_pocket_continuing_learning_output(pocket_cube_linear_policy_output)
  ╠═╡ =#

# ╔═╡ b5f507c6-4d9f-4c05-9db6-25c356cf24a0
#=╠═╡
pocket_cube_curriculum_continuing_dp_λ_linear_test.update_parameters!(4, 2f0 ^-9, 2f0 ^-10, 0.5f0; num_steps = 100_000) |> output -> display_pocket_continuing_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#

# ╔═╡ fe324bf7-9b9a-44d7-bece-27d6ec2d5b12
#=╠═╡
pocket_cube_curriculum_continuing_actor_critic_linear_test.update_parameters!(4, 0.001f0, 0.0001f0, 0.0002f0, 0.5f0, 0.5f0; num_steps = 1_000_000) |> output -> display_pocket_continuing_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#

# ╔═╡ 138a444c-9be7-4148-9744-957b66e2e658
#=╠═╡
pocket_cube_curriculum_continuing_dp_λ_fcann_test.update_parameters!(3, 1f-3, 0.001f0, 0.5f0; num_steps = 100_000) |> output -> display_pocket_continuing_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#

# ╔═╡ 26bd645a-3465-4cd8-83a5-b6cc970b8e37
#=╠═╡
display_pocket_continuing_learning_output(pocket_actor_critic_fcann_results; min_scramble = 1, max_scramble = 11)
  ╠═╡ =#

# ╔═╡ 46979441-fa1f-4fc6-a8ac-011ef12e4288
function get_pocket_continuing_stats(output::NamedTuple; min_scramble = 1, max_scramble = 11, kwargs...)
	f_stats = haskey(output, :policy_sample_action) ? get_pocket_policy_scramble_statistics : get_pocket_value_scramble_statistics
	f_stats(output, min_scramble, max_scramble; kwargs...)
end

# ╔═╡ 03e9de4d-bfdc-4c57-b17f-3e3ff08a6b63
#=╠═╡
function pocket_cube_nonlinear_value_train_curriculum(α::Float32, λ::Float32, hidden_layers::Vector{Int64}, reslayers::Integer; init_scramble::Integer = 2, kwargs...)
	output = pocket_cube_nonlinear_value_train_scramble(α, λ, hidden_layers, reslayers, init_scramble, init_scramble; kwargs...)
	hit_target = output.hit_target

	if !hit_target
		@info "Failed to reach performance target with initial scramble of $init_scramble moves"
		return output
	end
	
	scramble = init_scramble
	while hit_target
		scramble += 1
		output = pocket_cube_nonlinear_value_train_scramble(α, λ, hidden_layers, reslayers, scramble, scramble; kwargs...)
		hit_target = output.hit_target
	end

	stats = get_pocket_continuing_stats(output)
	CSV.write("pocket_cube_curriculum_fcann_dp_nonlinear_value_$(scramble)_scramble_stats_576_input_$(hidden_layers)_hidden_$(reslayers)_reslayers_9_actions.csv", DataFrame(stats))

	@info "Failed to reach performance target on a pure scramble of $scramble scramble moves.  Proceeding with mixed scrambles"
	scramble1 = scramble - 1
	scramble2 = scramble
	output2 = pocket_cube_nonlinear_value_train_scramble(α, λ, hidden_layers, reslayers, scramble1, scramble2; kwargs...)
	
	hit_target = output2.hit_target

	stats = get_pocket_continuing_stats(output2)
	CSV.write("pocket_cube_curriculum_fcann_dp_nonlinear_value_$(scramble1)_to_$(scramble2)_scramble_stats_576_input_$(hidden_layers)_hidden_$(reslayers)_reslayers_9_actions.csv", DataFrame(stats))

	if !hit_target
		@info "Failed to reach performance target with mixed scramble of $scramble1 to $scramble2 moves"
		return (;output..., scramble_fail = scramble)
	else
		@info "Successfully reached performance target with mixed scramble of $scramble1 to $scramble2 moves"
		return (;output2..., scramble_fail = scramble+1)
	end
end
  ╠═╡ =#

# ╔═╡ 5e560d61-b59a-4b53-b71d-54d5d0a8c86c
#=╠═╡
# const pocket_cube_nonlinear_value_output = pocket_cube_nonlinear_value_training(4f-2, 0.25f0, fill(2, 8), 1, 6, 6, 0; use_gpu = false, ϵ = 0.001f0, α_r̄ = 0.1f0, l2 = 0.1f0)

# const pocket_cube_nonlinear_value_output = pocket_cube_nonlinear_value_train_scramble(4f-2, 0.25f0, fill(512, 8), 1, 6, 6; use_gpu = true, ϵ = 0.001f0, α_r̄ = 0.1f0, l2 = 0.1f0)

const pocket_cube_nonlinear_value_output = pocket_cube_nonlinear_value_train_curriculum(4f-2, 0.25f0, fill(4096, 8), 1; use_gpu = true, ϵ = 0.001f0, α_r̄ = 0.1f0, l2 = .0f0, dropout = 0.0f0) #, init_scramble = 6)
  ╠═╡ =#

# ╔═╡ f4d2ca65-f13f-46be-91cd-7f5b3eff952f
#=╠═╡
display_pocket_continuing_learning_output(pocket_cube_nonlinear_value_output)
  ╠═╡ =#

# ╔═╡ d00391e4-5728-4699-97b0-470d978120ab
#=╠═╡
get_pocket_continuing_stats(pocket_cube_nonlinear_value_output)
  ╠═╡ =#

# ╔═╡ 572e3d20-ddcf-4dbe-91f8-0cd4a0e8fec2
#=╠═╡
runepisode(create_curriculum_pocket_episodic_mdp(2); π = pocket_cube_curriculum_continuing_actor_critic_linear_test.update_parameters!(2, 1f-3, 1f-3, 2f-2, 0.25f0, 0.85f0; num_steps = 1_000).policy_sample_action)
  ╠═╡ =#

# ╔═╡ 761319c6-ce9e-4896-952f-60044ac6458a
#this version of the problem uses the 9 move set instead of 3 to make it match the other solution step times
function build_tabular_pocket_mdp2()
	nstates = length(pocket_states)
	state_transition_map = zeros(Int64, 9, nstates)
	reward_transition_map = zeros(Float32, 9, nstates)
	i_s_term = pocket_state_index[solved_pocket_cube]
	@info "Building state and reward transition maps"
	for s in pocket_states
		i_s = pocket_state_index[s]
		if i_s == i_s_term
			...
			for i_a in eachindex(all_pocket_moves)
				s′ = rotate_pocket_cube2(s, i_a)
				i_s′ = pocket_state_index[s′]
				state_transition_map[i_a, i_s] = i_s′
				reward_transition_map[i_a, i_s] = -1f0
	...
	TabularMDP(pocket_states, all_pocket_moves, TabularDeterministicTransition(state_transition_map, reward_transition_map), () -> rand(eachindex(pocket_states)); state_index = pocket_state_index)
end

# ╔═╡ db664c9b-9a80-4e54-aeac-21ee11442005
const pocket_mdp2 = build_tabular_pocket_mdp2()

# ╔═╡ 50504f2f-dd90-4926-bd26-588bff555520
const pocket_cube_solution2 = value_iteration_v(pocket_mdp2, 1f0; usethreads=true, show_message = false, make_final_policy = TabularRL.make_greedy_bit_policy)

# ╔═╡ baadc879-0f2d-40a2-a2e9-db63bdc07dc8
const pocket_distance_count = [count(x -> x == v, pocket_cube_solution2.final_value) for v in -11:0]

# ╔═╡ f3d5c8fa-8faf-47b5-805e-fbd501cda7f8
#=╠═╡
DataFrame((moves_until_solution = 12 - i, number_of_states = pocket_distance_count[i]) for i in 1:12)
  ╠═╡ =#

# ╔═╡ f27b1b22-df76-47de-b48f-9f05c37d0c52
#=╠═╡
function display_pocket_episodic_mrp_learning_output(result::NamedTuple; min_scramble = 1, max_scramble = 11, kwargs...)
	output = result.output
	error_plot = output.episode_errors |> v -> plot_rewards(v, 100, 1000)
	stats = get_pocket_value_scramble_statistics(output, min_scramble, max_scramble; kwargs...)
	num_params = calculate_num_params(result.params)
	df = DataFrame((;stats[i]..., coverage = num_params / sum(pocket_distance_count[12-i:12])) for i in 1:length(stats))
	@htl("""
	Number of Parameters: $num_params vs $(length(pocket_states)) total states, $(round(100*num_params / length(pocket_states); sigdigits = 2)) % 
	<div style = "display: flex;">
	<div style = "width: 75%">
	$error_plot
	</div>
...
# ╔═╡ 9e420d06-af22-4343-b222-a09d6d509146
extrema(pocket_cube_solution2.final_value)

# ╔═╡ 7a72b5fa-46ee-46eb-991f-544feaa19d8c
const pocket_tabular_policy_lookup = [findfirst(!iszero, a) for a in eachcol(pocket_cube_solution2.optimal_policy)]

# ╔═╡ c230b2fe-c02a-4b09-8765-d8557a97eabe
const pocket_tabular_transition_lookup = [pocket_mdp2.ptf.state_transition_map[pocket_tabular_policy_lookup[i_s], i_s] for i_s in eachindex(pocket_states)]

# ╔═╡ e349ee8c-536a-40df-827a-85ab491abf8d
const pocket_state_transition_lookup = Dict(pocket_states[i_s] => pocket_states[pocket_tabular_transition_lookup[i_s]] for i_s in eachindex(pocket_states))

# ╔═╡ 9e3cb3aa-62d3-48f7-abfb-f54e75c2f8db
begin
	pocket_step_ideal(s::AbstractVector{I}) where I <: Integer = pocket_state_transition_lookup[s]
	pocket_step_ideal(s::NamedTuple) = pocket_step_deal(s.cube)
end

# ╔═╡ cc9f7ab8-d65d-48dc-bc92-f9ad02eab78b
function create_pocket_episodic_mrp(;min_scramble::Integer = 11, max_scramble::Integer = 11)
	isterm(s) = s == solved_pocket_cube
	initialize_state() = initialize_pocket_cube(rand(min_scramble:max_scramble))

	function step(s)
		s′ = pocket_state_transition_lookup[s]
		r = (-1f0 + isterm(s′)*6f0) * 1.75f0 / 5f0
		(r, s′)
	end
...
# ╔═╡ fe8312c1-e0f2-4d8a-b6e6-a5de81864d92
function initialize_pocket_episodic_mrp_linear_test(;feature_vector = init_pocket_feature(), kwargs...)

	params = initialize_linear_parameters(length(feature_vector), 0f0)
	
	function f!(num_episodes, λ, α; min_scramble::Integer = 11, max_scramble::Integer = 11, γ = 1f0, kwargs...)
		mrp = create_pocket_episodic_mrp(;min_scramble = min_scramble, max_scramble = max_scramble)
		output = semi_gradient_TDλ_linear(mrp, γ, λ, num_episodes, typemax(Int64), deepcopy(feature_vector), update_pocket_feature!; α = α, parameters = params, kwargs...)
		q̂, form_kwargs = NonTabularRL.form_value_function(create_curriculum_pocket_episodic_mdp(11), γ, update_pocket_feature!, output.value_function, deepcopy(feature_vector), output.parameters)
		(output = (episode_errors = output.episode_history.errors, value_function = q̂, form_kwargs = form_kwargs), params = deepcopy(params))
	end
	(parameters = params, update! = f!)
end

# ╔═╡ 18067756-5e8d-4c94-b028-5bb3f5c31fa3
const td_λ_episodic_mrp_linear_test = initialize_pocket_episodic_mrp_linear_test()

# ╔═╡ 81bc53ca-1118-44fc-82b4-d1b2d4fe8418
const td_λ_episodic_mrp_linear_output = td_λ_episodic_mrp_linear_test.update!(1_000_000, 0.99f0, 1f-3; min_scramble = 11, max_scramble = 11)

# ╔═╡ db9cc405-2252-4c8a-b428-7560b4c24f75
#=╠═╡
display_pocket_episodic_mrp_learning_output(td_λ_episodic_mrp_linear_output; min_scramble = 1, max_scramble = 11)
  ╠═╡ =#

# ╔═╡ f0a020a4-c083-47e2-9622-f89122c935c8
#=╠═╡
function initialize_pocket_episodic_mrp_nonlinear_test(layers::Vector{Int64}; feature_vector = init_pocket_feature(), kwargs...)
	fname = "pocket_mrp_td_λ_fcann_params_$(string(layers))_hidden_1_reslayer.bin"
	
	params = if haskey(td_λ_fcann_episodic_mrp_mastery_results, (layers = layers,)) 
		deepcopy(td_λ_fcann_episodic_mrp_mastery_results[(layers = layers,)].params) 
	elseif isfile(fname)
		weights = first(FCANN.readBinParams(fname))
		(weights = weights, reslayers = 1)
	else
		NonTabularRL.initialize_fcann_params(length(feature_vector), layers, 1, 1, true)
	end
	
	function f!(num_episodes, λ, α; min_scramble::Integer = 11, max_scramble::Integer = 11, γ = 1.0f0, feature_vector = feature_vector, kwargs...)
		mrp = create_pocket_episodic_mrp(;min_scramble = min_scramble, max_scramble = max_scramble)
		output = semi_gradient_TDλ_fcann(mrp, γ, λ, num_episodes, typemax(Int64), deepcopy(feature_vector), update_pocket_feature!, layers; α = α, parameters = params, kwargs...)
		q̂, form_kwargs = NonTabularRL.form_value_function(create_curriculum_pocket_episodic_mdp(11), γ, update_pocket_feature!, output.value_function, deepcopy(feature_vector), output.parameters)
		
		output1 = (episode_errors = output.episode_history.errors, value_function = q̂, form_kwargs = form_kwargs)
		output2 = (output = output1, params = deepcopy(params))
		td_λ_fcann_episodic_mrp_mastery_results[(layers = layers,)] = output2
		return output2
	end

	function f_exhaustive!(trial_episodes, λ, α; kwargs...)
		@info "Training with learning rate $α until error fails to improve"
		results1 = f!(1_000, λ, 0f0; kwargs...)
		err1 = mean(results1.output.episode_errors)
		@info "Reference error is $err1"

		l = round(Int64, trial_episodes / 2)
		results2 = f!(trial_episodes, λ, α; kwargs...)
		err2 = mean(view(results2.output.episode_errors, l:trial_episodes))
		trial = 2
		while err2 < err1
			@info "On trial $trial error improved from $err1 to $err2"
			err1 = err2
			results1 = results2
			results2 = f!(trial_episodes, λ, α; kwargs...)
			trial += 1
		...
		@info "Concluded training with learning rate of $α after $(trial-1) trials with an error of $err1"
		copy!(params, results1.params)
		return results1
	end
	
	function f_rate_decay!(trial_episodes, λ, α_init; kwargs...)
		@info "Training with an initial learning rate of $α_init and decaying by 50% until failure to improve"
		α = α_init
		l = round(Int64, trial_episodes / 2)
		results1 = f_exhaustive!(trial_episodes, λ, α; kwargs...)
		err1 = if length(results1.output.episode_errors) ≠ trial_episodes
			mean(results1.output.episode_errors)
		else
			mean(view(results1.output.episode_errors, l:trial_episodes))
		end

		α /= 2
		results2 = f_exhaustive!(trial_episodes, λ, α; kwargs...)
		err2 = if length(results2.output.episode_errors) ≠ trial_episodes
			mean(results2.output.episode_errors)
		else
			mean(view(results2.output.episode_errors, l:trial_episodes))
		end

		trial = 2
		while err2 < err1
			@info "After $trial rounds of learning rate decay, error improved from $err1 to $err2"
		...
		@info "Concluded after $(trial - 1) rounds of learning rate decay with a learning rate of $(α*2) and an error of $err1"
		copy!(params, results1.params)
		return results1
	end

		
	(parameters = params, update! = f!, update_exhaustive! = f_exhaustive!, update_rate_decay! = f_rate_decay!)
end
  ╠═╡ =#

# ╔═╡ 1980337f-59a6-4aa5-ade3-ed9e6b9ce1ed
#=╠═╡
const td_λ_episodic_mrp_nonlinear_test = initialize_pocket_episodic_mrp_nonlinear_test(td_λ_episodic_mrp_nonlinear_layers)
  ╠═╡ =#

# ╔═╡ 3c95d2cd-f5d6-4526-8cbe-0be835b387e7
#=╠═╡
const td_λ_episodic_mrp_nonlinear_output = td_λ_episodic_mrp_nonlinear_test.update_rate_decay!(1_000, 0.99f0, 4f-3; min_scramble = 11, max_scramble = 11)#, use_gpu=true, feature_vector = init_pocket_dense_feature())
  ╠═╡ =#

# ╔═╡ 3aaa666c-54fe-44eb-b730-7b2b599bf5a3
#=╠═╡
begin
	td_λ_episodic_mrp_nonlinear_output
	@bind td_λ_fcann_episodic_mrp_mastery_layer_select Select(collect(keys(td_λ_fcann_episodic_mrp_mastery_results)), default = (layers = td_λ_episodic_mrp_nonlinear_layers,))
end
  ╠═╡ =#

# ╔═╡ 9d0e57b6-ec9a-44a4-9e1e-643f780d12cb
#=╠═╡
display_pocket_episodic_mrp_learning_output(td_λ_fcann_episodic_mrp_mastery_results[td_λ_fcann_episodic_mrp_mastery_layer_select]; min_scramble = 1, max_scramble = 11, ntrials = 100)
  ╠═╡ =#

# ╔═╡ 072e7bc4-5db1-43b4-8809-77cd6b2125cd
#=╠═╡
td_λ_fcann_episodic_mrp_mastery_results[td_λ_fcann_episodic_mrp_mastery_layer_select].output.value_function(create_curriculum_pocket_episodic_mdp(8).initialize_state())
  ╠═╡ =#
...
function check_next_moves_recur2(current_cube, trajectory::Vector{Tuple{Int64, Int64}}, cubes::Vector{Vector{UInt8}}, cube′::Vector{UInt8}, depth::Integer, threshold::Integer, states_checked; show_message = true)
	if (current_cube == solved_pocket_cube)
		show_message && @info "Found a solution with $depth moves after checking $(states_checked[1]) states"
		return (true, trajectory[1:depth])
	end
	...
	for m in next_moves
		...
		trajectory[depth + 1] = m
		states_checked[1] += 1
		(found, result) = check_next_moves_recur2(cubes[depth+1], trajectory, cubes, cube′, depth+1, threshold, states_checked; show_message = show_message)
		found && return (true, result)
		min_overshoot = min(min_overshoot, result)
	end
...
function check_next_moves2(cube::SVector{24, UInt8}, cube′::Vector{UInt8}, maxdepth::Integer; show_message::Bool = true)
	...
	while threshold <= maxdepth
		show_message && @info "Starting search round with threshold: $threshold"
		found, result = check_next_moves_recur2(cube, trajectory, cubes, cube′, 0, threshold, states_checked; show_message = show_message)
		if found
			l = length(result)
			return (l, result, cubes[1:l], states_checked[1])
		else
	...
	show_message && @info "No solution found within maximum depth of $maxdepth"
	# return (best_depth, best_trajectory[1:best_depth[1]], best_cubes[1:best_depth[1]])
end

# ╔═╡ 435df0bb-ac20-4b6a-9575-03cd249b604c
function solve_pocket_cube_ida_star(s::SVector{24, UInt8}; maxdepth::Integer = 12, kwargs...)
	cube′ = Vector(s)
	check_next_moves2(s, cube′, maxdepth; kwargs...)
end

# ╔═╡ dd790cf7-12b4-4cef-8d9d-5605e52c1b2c
...
# ╔═╡ cbb7f7fc-8924-48d0-81f7-e9aa7c888d04
#=╠═╡
[render_pocket_cube(pocket_states[i_s]) for i_s in test_episode[1]]
  ╠═╡ =#

# ╔═╡ def39521-a9fe-4b95-9d8e-a3af4be878ce
#=╠═╡
initialize_pocket_cube(11) |> render_pocket_cube
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "6b14b66197baec26a0f6b9ab686a08661f96e43b"
