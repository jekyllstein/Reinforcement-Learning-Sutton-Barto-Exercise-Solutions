...
# ╔═╡ 8358ed2d-de66-41cf-80f5-751e10ebee39
#use gpu acceleration for generating statistics if possible although worry about parallel execution, more focus on using it in the exhaustive training.  Write full training routine that calculates the ideal performance and ramps up training steps up to 1 million and then procedes to a higher scramble number and then saving the performance statistics at the very end.
...
# ╔═╡ 70d4a71c-f92e-4ea6-adb1-e1bf3a007ccb
(cube = solved_pocket_cube, scramble_moves = 5, move_count = 0) |> typeof
...
# ╔═╡ 69c0918b-eaf2-43a9-a5e4-b89a27f6fb0b
function load_pocket_curriculum_dp_λ_params(hidden, reslayers)
	name = "pocket_cube_curriculum_dp_λ_fcann_params_$(string(hidden))_$(reslayers)_reslayer.bin"
	if isfile(name)
		...
		initialize_fcann_params(24*24, hidden, 1, reslayers, true)
	end
end

# ╔═╡ 60602fb2-889b-41d4-a0ee-01baf72aca8f
#=╠═╡
@bind run_save_pocket_cube_curriculum_dp_λ_params CounterButton("Save DP Params")
  ╠═╡ =#

# ╔═╡ 3f7bb132-ab6c-4d97-bf01-3216a6a8720a
# ╠═╡ disabled = true
#=╠═╡
const pocket_cube_curriculum_continuing_dp_λ_fcann_results = pocket_cube_curriculum_continuing_dp_λ_fcann_test.update_parameters!(6, 4f-3, 0.01f0, 0.2f0; num_steps = 1_000_000, ϵ = 0.01f0, use_gpu = true, feature_vector = zeros(Float32, 24*24))
  ╠═╡ =#

# ╔═╡ 138a444c-9be7-4148-9744-957b66e2e658
#=╠═╡
display_pocket_continuing_learning_output(pocket_cube_curriculum_continuing_dp_λ_fcann_results; min_scramble = 1, max_scramble = 6)
  ╠═╡ =#

# ╔═╡ 460b4081-5197-4e41-85af-dc92b073f600
#=╠═╡
@bind run_save_pocket_cube_curriculum_ac_params CounterButton("Save Actor Critic Params")
  ╠═╡ =#

# ╔═╡ 4d5778c4-e82b-48b3-a2f2-c37737e2b6a3
# ╠═╡ disabled = true
#=╠═╡
const pocket_actor_critic_fcann_results = pocket_cube_curriculum_continuing_actor_critic_fcann_test.update_parameters!(2, 4f-2, 4f-2, 0.01f0, 0.f0, 0.f0; num_steps = 100_000, use_gpu = true, feature_vector = zeros(Float32, 24*24)) 
  ╠═╡ =#
...
#=╠═╡
pocket_actor_critic_fcann_results.policy_function((cube = initialize_pocket_cube(4), scramble_moves = 2, move_count = 0))
  ╠═╡ =#

# ╔═╡ 989e91f7-e331-4687-bf8c-1efbca516997
#=╠═╡
pocket_actor_critic_fcann_results.value_function((cube = initialize_pocket_cube(4), scramble_moves = 2, move_count = 0))
  ╠═╡ =#
...
# ╔═╡ 43d7394f-2cf2-42c4-b960-c91f608c57db
...
# ╔═╡ 8ad42499-869e-4e36-9121-44dc05a21e4c
#=╠═╡
function create_episodic_trial(f::Function)
	run_trial(args...; kwargs...) = mean(f(args...; kwargs...).episode_rewards)
end
  ╠═╡ =#
...
# ╔═╡ cf454cbe-7914-4b32-b893-f8ab2599b7c5
...
# ╔═╡ e1440046-9306-4279-abf7-3daadfd2fce9
...
const td_λ_episodic_mrp_nonlinear_layers = fill(4096, 8)
...
# ╠═╡ disabled = true
#=╠═╡
display_pocket_episodic_mrp_learning_output(td_λ_fcann_episodic_mrp_mastery_results[td_λ_fcann_episodic_mrp_mastery_layer_select]; min_scramble = 1, max_scramble = 11)
  ╠═╡ =#

# ╔═╡ 559125e5-28f3-4bd2-9441-098c8b654492
#=╠═╡
@bind save_fcann_mrp_params CounterButton("Save Parameters to Disk")
  ╠═╡ =#
...
# ╔═╡ f8dad5ef-4083-46ff-8ad5-271b3fccc84f
function save_pocket_cube_curriculum_dp_λ_params(params::FCANNParams)
	input_size, hidden, num_layers = get_network_dimensions(params)
	name = "pocket_cube_curriculum_dp_λ_fcann_params_$(string(hidden))_$(params.reslayers)_reslayer.bin"
	save_fcann_params(params, name)
end

# ╔═╡ 875ca26d-8ab6-4c96-adc9-b4db00770ec8
function save_pocket_cube_curriculum_ac_params(params::FCANNParams)
	input_size, hidden, num_layers = get_network_dimensions(params)
	name = "pocket_cube_curriculum_ac_fcann_params_$(string(hidden))_$(params.reslayers)_reslayer.bin"
	save_fcann_params(params, name)
end

# ╔═╡ 90419ec5-cf5c-431b-a3c7-9d83fd26879a
...
# ╔═╡ cd0d44c1-91d3-429a-8ff1-c0e49a5fc6cb
#=╠═╡
if save_fcann_mrp_params > 1
	save_episodic_pocket_mrp_fcann_params()
end
  ╠═╡ =#
...
# ╔═╡ 014ee21b-9b13-4bbc-9f55-96f9081736b9
function load_pocket_curriculum_ac_params(hidden, reslayers)
	name = "pocket_cube_curriculum_ac_fcann_params_$(string(hidden))_$(reslayers)_reslayer.bin"
	if isfile(name)
		v = FCANN.readBinParams(name)
		θs = v[1][1]
		βs = v[1][2]
		...
		initialize_fcann_params(24*24, hidden, length(all_pocket_moves), reslayers, true)
	end
end
...
function pocket_curriculum_sparse_continuing_step(s::@NamedTuple{cube::SVector{24, UInt8}, scramble_moves::Int64, move_count::Int64}, i_a; scramble_moves::Integer = 11)
...
function create_curriculum_pocket_continuing_mdp(num_moves::Integer)
	init_state() = (cube = initialize_pocket_cube(num_moves), scramble_moves = num_moves, move_count = 0)
	step(s, i_a) = pocket_curriculum_sparse_continuing_step(s, i_a; scramble_moves = num_moves)
...
begin
	function pocket_cube_curriculum_continuing_actor_critic_linear(scramble_moves::Integer, α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, kwargs...)
		...
		all(iszero, (λ_θ, λ_w)) && return one_step_actor_critic_linear(mdp, num_steps, init_pocket_feature(), update_pocket_feature!; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
...
#=╠═╡
begin
	function pocket_cube_curriculum_continuing_actor_critic_fcann(scramble_moves::Integer, hidden_layers, α_θ, α_w, α_r̄, λ_θ, λ_w; num_steps = 10_000, feature_vector = init_pocket_feature(), kwargs...)
		mdp = create_curriculum_pocket_continuing_mdp(scramble_moves)
		all(iszero, (λ_θ, λ_w)) && return one_step_actor_critic_fcann(mdp, num_steps, feature_vector, update_pocket_feature!, hidden_layers; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
		actor_critic_with_eligibility_traces_fcann(mdp, λ_θ, λ_w, num_steps, feature_vector, update_pocket_feature!, hidden_layers; α_θ = α_θ, α_w = α_w, α_r̄ = α_r̄, kwargs...)
	end

	function setup_pocket_cube_curriculum_continuing_actor_critic_fcann(hidden_layers, reslayers)
		policy_params = load_pocket_curriculum_ac_params(hidden_layers, reslayers)
	...
	function pocket_cube_curriculum_continuing_dp_λ_fcann(scramble_moves::Integer, hidden_layers, α, α_r̄, λ; num_steps = 10_000, feature_vector = init_pocket_feature(), kwargs...)
		mdp = create_curriculum_pocket_continuing_mdp(scramble_moves)
		dp_λ_fcann(mdp, λ, num_steps, feature_vector, update_pocket_feature!, hidden_layers; α = α, α_r̄ = α_r̄, kwargs...)
	end

	function setup_pocket_cube_curriculum_continuing_dp_λ_fcann(hidden_layers, reslayers)
		parameters = load_pocket_curriculum_dp_λ_params(hidden_layers, reslayers)
...
#=╠═╡
const pocket_cube_curriculum_continuing_dp_λ_fcann_test = setup_pocket_cube_curriculum_continuing_dp_λ_fcann(fill(512, 8), 1)
  ╠═╡ =#

# ╔═╡ 4e5c481b-6b1b-4d9a-a3dc-3724eef54e67
#=╠═╡
if run_save_pocket_cube_curriculum_dp_λ_params > 0 
	save_pocket_cube_curriculum_dp_λ_params(pocket_cube_curriculum_continuing_dp_λ_fcann_test.parameters)
end
  ╠═╡ =#

# ╔═╡ ca7479b5-543b-41ad-90e4-c81b8950b3d3
#=╠═╡
const pocket_cube_curriculum_continuing_actor_critic_fcann_test = setup_pocket_cube_curriculum_continuing_actor_critic_fcann(fill(512, 8), 1)
  ╠═╡ =#

# ╔═╡ 4592bea6-04b5-46d8-acb3-39db1cf5d63b
#=╠═╡
if run_save_pocket_cube_curriculum_ac_params > 0 
	save_pocket_cube_curriculum_ac_params(pocket_cube_curriculum_continuing_actor_critic_fcann_test.policy_parameters)
end
  ╠═╡ =#

# ╔═╡ bdb84292-c494-41a7-8a61-4cd14d8fc5f1
function pocket_curriculum_sparse_episodic_step(s::@NamedTuple{cube::SVector{24, UInt8}, scramble_moves::Int64, move_count::Int64}, i_a; scramble_moves::Integer = 11)
	cube′ = rotate_pocket_cube2(s.cube, i_a)
	r = Float32(cube′ == solved_pocket_cube)
	return (r, (cube = cube′, scramble_moves = s.scramble_moves, move_count = s.move_count + 1))
end

# ╔═╡ f7a13cab-2e55-4218-bb09-f919bdbdb8d3
function pocket_curriculum_dense_episodic_step(s::@NamedTuple{cube::SVector{24, UInt8}, scramble_moves::Int64, move_count::Int64}, i_a; scramble_moves::Integer = 11)
	cube′ = rotate_pocket_cube2(s.cube, i_a)
	r = -1f0 + (cube′ == solved_pocket_cube)*10f0
	return (r, (cube = cube′, scramble_moves = s.scramble_moves, move_count = s.move_count + 1))
end

# ╔═╡ 392bdf01-31ac-4814-ba20-6c24b6396b3d
function create_curriculum_pocket_episodic_mdp(num_moves::Integer)
	init_state() = (cube = initialize_pocket_cube(num_moves), scramble_moves = num_moves, move_count = 0)
	step(s, i_a) = pocket_curriculum_dense_episodic_step(s, i_a; scramble_moves = num_moves)
	function isterm(s)
...
# ╔═╡ f996be9f-eb74-408a-b331-88a62e7435c8
mdp = create_curriculum_pocket_episodic_mdp(11)

# ╔═╡ 345acba4-1881-4369-9bdc-1cb5040678fc
function get_pocket_scramble_statistic(scramble::Integer, π::Function, ntrials::Integer)
	mdp = create_curriculum_pocket_episodic_mdp(scramble)
	1:ntrials |> Map(i -> runepisode(mdp; π = π) |> out -> out[4].cube == solved_pocket_cube) |> foldxt(+) |> x -> x / ntrials
end
...
function display_pocket_continuing_learning_output(output::NamedTuple; min_scramble = 1, max_scramble = 11)
	reward_plot = output.reward_history |> v -> plot_rewards(v, 100, 1000)
...
#=╠═╡
pocket_cube_curriculum_continuing_dp_λ_linear_test.update_parameters!(3, 2f0 ^-9, 2f0 ^-10, 0.5f0; num_steps = 100_000) |> output -> display_pocket_continuing_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#

# ╔═╡ fe324bf7-9b9a-44d7-bece-27d6ec2d5b12
#=╠═╡
pocket_cube_curriculum_continuing_actor_critic_linear_test.update_parameters!(3, 2f-3, 1f-4, 0.01f0, 0.0f0, 0.0f0; num_steps = 100_000) |> output -> display_pocket_continuing_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#

# ╔═╡ 8d8e4462-cc3f-4053-bd71-ef0fe85a1a28
#=╠═╡
function display_pocket_episodic_learning_output(output::NamedTuple; min_scramble = 1, max_scramble = 11, nsmooth = 100)
	reward_plot = output.episode_rewards |> v -> plot_rewards(v, nsmooth, 1000)
	f_stats = haskey(output, :policy_sample_action) ? get_pocket_policy_scramble_statistics : get_pocket_value_scramble_statistics
	stats = f_stats(output, min_scramble, max_scramble)
	@htl("""
	<div style = "display: flex;">
...
# ╔═╡ c74c5542-b9db-4702-9745-8020cd997418
#=╠═╡
begin
	function pocket_cube_curriculum_episodic_actor_critic_linear(scramble_moves::Integer, α_θ, α_w, λ_θ, λ_w; num_steps = 10_000, kwargs...)
		mdp = create_curriculum_pocket_episodic_mdp(scramble_moves)
		actor_critic_with_eligibility_traces_linear(mdp, 1f0, λ_θ, λ_w, typemax(Int64), num_steps, init_pocket_feature(), update_pocket_feature!; α_θ = α_θ, α_w = α_w, kwargs...)
	end

	function setup_pocket_cube_curriculum_episodic_actor_critic_linear()
		policy_params = initialize_linear_parameters(24*24, length(all_pocket_moves), 0f0)
		value_params = initialize_linear_parameters(24*24, 0f0)
		update_parameters!(scramble_moves, α_θ, α_w, λ_θ, λ_w; kwargs...) = pocket_cube_curriculum_episodic_actor_critic_linear(scramble_moves, α_θ, α_w, λ_θ, λ_w; kwargs..., policy_params = policy_params, value_params = value_params)
		return (policy_parameters = policy_params, value_parameters = value_params, update_parameters! = update_parameters!)
	end

	function pocket_cube_curriculum_episodic_dp_λ_linear(scramble_moves::Integer, α, λ; num_steps = 10_000, kwargs...)
		mdp = create_curriculum_pocket_episodic_mdp(scramble_moves)
		dp_λ_linear(mdp, 1f0, λ, typemax(Int64), num_steps, init_pocket_feature(), update_pocket_feature!; α = α, kwargs...)
	end

	function setup_pocket_cube_curriculum_episodic_dp_λ_linear()
		parameters = initialize_linear_parameters(24*24, 0f0)
		update_parameters!(scramble_moves, α, λ; kwargs...) = pocket_cube_curriculum_episodic_dp_λ_linear(scramble_moves, α, λ; kwargs..., parameters = parameters)
		return (parameters = parameters, update_parameters! = update_parameters!)
	end
		

	const pocket_cube_curriculum_episodic_actor_critic_linear_parameter_study = setup_parameter_study(create_episodic_trial(pocket_cube_curriculum_episodic_actor_critic_linear), (:scramble_moves, :α_θ, :α_w, :λ_θ, :λ_w), (num_steps = 10_000, trace_type = AccumulatingTrace()))

	const pocket_cube_curriculum_episodic_dp_λ_linear_parameter_study = setup_parameter_study(create_episodic_trial(pocket_cube_curriculum_episodic_dp_λ_linear), (:scramble_moves, :α, :λ), (num_steps = 10_000, trace_type = AccumulatingTrace()))
end
  ╠═╡ =#

# ╔═╡ 298751d5-8932-4aae-8596-b69f96479351
#=╠═╡
const pocket_cube_curriculum_episodic_dp_λ_linear_test = setup_pocket_cube_curriculum_episodic_dp_λ_linear()
  ╠═╡ =#

# ╔═╡ 9c0eb359-f9f7-4afe-92d8-b7a4d683240d
#=╠═╡
pocket_cube_curriculum_episodic_dp_λ_linear_test.update_parameters!(2, 2f0 ^-12, 0.99f0; num_steps = 100_000, trace_type = ReplacingTrace()) |> output -> display_pocket_episodic_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#

# ╔═╡ ed1911e4-d6ec-46bd-a811-2291b26fa0e0
#=╠═╡
const pocket_cube_curriculum_episodic_actor_critic_linear_test = setup_pocket_cube_curriculum_episodic_actor_critic_linear()
  ╠═╡ =#

# ╔═╡ e8f68664-dd2f-498c-937a-b32d48015d90
#=╠═╡
pocket_cube_curriculum_episodic_actor_critic_linear_test.update_parameters!(2, 2f-8, 2f-8, 0.5f0, 0.5f0; num_steps = 100_000) |> output -> display_pocket_episodic_learning_output(output; min_scramble = 1, max_scramble = 5)
  ╠═╡ =#
...
#=╠═╡
function display_pocket_episodic_mrp_learning_output(result::NamedTuple; min_scramble = 1, max_scramble = 11)
	output = result.output
	error_plot = output.episode_errors |> v -> plot_rewards(v, 100, 1000)
	stats = get_pocket_value_scramble_statistics(output, min_scramble, max_scramble)
...
#=╠═╡
function initialize_pocket_episodic_mrp_nonlinear_test(layers::Vector{Int64}; feature_vector = init_pocket_feature(), new_params = false, kwargs...)
	fname = "pocket_mrp_td_λ_fcann_params_$(string(layers))_hidden_1_reslayer.bin"
	
	params = if haskey(td_λ_fcann_episodic_mrp_mastery_results, (layers = layers,)) && !new_params 
		deepcopy(td_λ_fcann_episodic_mrp_mastery_results[(layers = layers,)].params) 
	elseif isfile(fname) && !new_params
	...
	function f_exhaustive!(trial_episodes, λ, α; kwargs...)
		check_bad_params(params) && error("Cannot begin training with bad parameter values")
		backup_params = copy(params)
		@info "Training with learning rate $α until error fails to improve"
		results1 = f!(1_000, 0f0, 0f0; kwargs...)
		if check_bad_params(params)
			copy!(params, backup_params)
			error("Cannot begin training with bad parameter values")
		end
		...
		trial = 2
		while (err2 < err1) && !check_bad_params(params)
...
#=╠═╡
const td_λ_episodic_mrp_nonlinear_test = initialize_pocket_episodic_mrp_nonlinear_test(td_λ_episodic_mrp_nonlinear_layers; new_params = true)
  ╠═╡ =#

# ╔═╡ 3c95d2cd-f5d6-4526-8cbe-0be835b387e7
#=╠═╡
const td_λ_episodic_mrp_nonlinear_output = td_λ_episodic_mrp_nonlinear_test.update_rate_decay!(100, 0.9f0, 1f-2; min_scramble = 11, max_scramble = 11, use_gpu=true, feature_vector = zeros(Float32, 24*24), l2 = 0.01f0)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "47b0a37e7e3e8cdd3553398dd6bd966170127769"
