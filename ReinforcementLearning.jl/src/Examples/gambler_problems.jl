...
function create_gambler_mrp(s0::Integer, sgoal::Integer, p::Float32)
	states = collect(0:sgoal)
	l = length(states)
	state_transition_map = [SparseVector(zeros(Float32, l)) for _ in 1:l]
	reward_transition_map = [zeros(Float32, 2) for _ in 1:l]
	reward_transition_map[l-1] = [0f0, 1f0]
	reward_transition_map[1] = [0f0]
	reward_transition_map[l] = [0f0]
	state_transition_map[1][1] = 1f0
	state_transition_map[l][l] = 1f0
	for i_s in 2:l-1
		state_transition_map[i_s][i_s+1] = p
		state_transition_map[i_s][i_s-1] = 1-p
	end
...
function analyze_gambler_problem(s0, sgoal, p)
	...
	values = round.(mrp_evaluation(mrp, 1f0).value_function[2:end-1]; sigdigits = 3)
	value_trace = bar(x = 1:sgoal-1, y = values, name = "Win Probability")
	ref_trace = scatter(x = 0:sgoal, y = fill(0.5, sgoal+1), mode = "lines", line_color = "gray", line_dash = "dash", name = "")
...
# ╔═╡ 507fc57c-804d-4ba0-8032-87901910d132
html"""
	<style>
		main {
			margin: 0 auto;
			max-width: min(1600px, 99%);
...
manifest_format = "2.0"
project_hash = "ba88f07b8733c86b218b5d5e7f8c0d5607bf5fed"
