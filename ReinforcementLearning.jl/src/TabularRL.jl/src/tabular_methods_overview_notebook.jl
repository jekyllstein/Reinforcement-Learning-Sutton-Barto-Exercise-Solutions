...
#=╠═╡
begin
	using PlutoUI, PlutoPlotly, ProfileCanvas, HypertextLiteral, BenchmarkTools
...
function sample_action(v::NTuple{N, T}) where {N, T<:Real} 
	i_a = 0
	maxv = typemin(T)
	e = eps(T)
	c = one(T) - 2*e
...
"""
function sample_action(π::AbstractMatrix{<:Real}, i_s::Integer)
	(n, m) = size(π)
...
begin
	find_available_actions(m::Array{<: Integer, N}) where N = BitArray(m .!= 0)
...
begin
	...
	abstract type AbstractMP{T<:Real, S, P<:AbstractTransition, F} end
	abstract type AbstractMDP{T<:Real, S, A, P <: AbstractTransition{T, 2}, F} <: AbstractMP{T, S, P, F} end
	abstract type AbstractMRP{T<:Real, S, P <: AbstractTransition{T, 1}, F} <: AbstractMP{T, S, P, F} end
	...
	struct TabularMDP{T<:Real, S, A, P <: AbstractTabularTransition{T, 2}, F} <: AbstractMDP{T, S, A, P, F}
		states::Vector{S}
		actions::Vector{A}
		ptf::P
		initialize_state_index::F #function which provides an initial state index or a distribution over starting states
	...
	TabularMDP(states, actions, args::Vararg{Any, N}; kwargs...) where N = TabularMDP(Vector(states), Vector(actions), args...; kwargs...)
end
...
begin
	...
	```
	"""
	initialize_state_action_value(mdp::TabularMDP{T}; init_value::T = zero(T)) where {T<:Real} = ones(T, length(mdp.actions), length(mdp.states)) .* init_value
...
begin
	...
	"""
	initialize_state_value(mdp; kwargs...) = initialize_state_value(mdp.ptf; kwargs...)
	initialize_state_value(mrp::TabularMRP{T}; init_value::Real = zero(T)) where {T<:Real} = ones(T, length(mrp.states)) .* T(init_value)
...
begin
	function runepisode!((states, rewards)::Tuple{Vector{Int64}, Vector{T}}, mdp::TabularMRP{T}; i_s0::Integer = initialize_state_index(mdp), max_steps = Inf) where {T<:Real}
	...
	function runepisode(mdp::TabularMRP{T}; kwargs...) where {T<:Real}
		states = Vector{Int64}()
		rewards = Vector{T}()
...
# ╔═╡ 4c299506-b7f6-477c-9c49-e9ea01ea9244
function make_greedy_bit_policy(Q::Matrix{T}, args...) where T<:Real
	π = BitMatrix(undef, size(Q)...)
	for i_s in 1:size(Q, 2)
		qmax = typemin(T)
		@inbounds @simd for i_a in 1:size(Q, 1)
			qmax = max(qmax, Q[i_a, i_s])
		end

		@inbounds @simd for i_a in 1:size(Q, 1)
			π[i_a, i_s] = isequal(Q[i_a, i_s], qmax)
		end
	end
	return π
end

# ╔═╡ cbf6d7b0-fc72-416c-801a-a9e723ecd2d4
make_greedy_bit_policy(V::Vector{T}, ptf::TabularTransitionDistribution) where T<:Real = make_greedy_bit_policy(V, ptf, one(T))
...
begin
	function copy_state_distribution!(μ::Vector{T}, μ0::AbstractVector{T}) where T<:Real
		μ .= μ0
		return μ
	end

	function copy_state_distribution!(μ::Vector{T}, μ0::Union{Set{<:Integer}, AbstractVector{<:Integer}}) where T<:Real
		μ .= zero(T)
		l = length(μ0)
		p = one(T) / l
		for i_s in μ0
	...
	function copy_state_distribution!(μ::Vector{T}, μ0::Integer) where T<:Real
		μ .= zero(T)
		μ[μ0] = one(T)
		return μ
	end
...
function calculate_μ_episodic(mdp::TabularMDP{T, <:Any, <:Any, <:TabularTransitionDistribution, <:Union{AbstractVector{<:Integer}, Integer, Set{<:Integer}}}, π; θ = sqrt(eps(T)), maxiter = 100) where T<:Real
	...
	return (total_visitations = accumulated_μ, iterations = iter, final_error = delt)
end
...
begin
	function bellman_optimal_update!(Q::Matrix{T}, i_s::Int64, i_a::Int64, ptf::TabularTransitionDistribution{T, ST, RT}, γ::T; calc_delt::Function = calc_pct_change) where {T <: Real, ST, RT}
		q = bellman_state_action_value(ptf, i_s, i_a, γ, Q)	
		delt = calc_delt(Q[i_a, i_s], q)
		Q[i_a, i_s] = q
		return delt
	end

	function bellman_optimal_update!(V::Vector{T}, i_s::Int64, ptf::TabularTransitionDistribution{T, ST, RT}, γ::T; calc_delt::Function = calc_pct_change) where {T <: Real, ST, RT}
		(num_actions, num_states) = size(ptf.state_transition_map)
		x = typemin(T)
		@inbounds @simd for i_a in 1:num_actions
			x = max(x, bellman_state_action_value(ptf, i_s, i_a, γ, V))
		end
		delt = calc_delt(V[i_s], x)
		V[i_s] = x
		return delt
	end

	function bellman_optimal_update!(Q′::Matrix{T}, Q::Matrix{T}, i_s::Int64, i_a::Int64, ptf::TabularTransitionDistribution{T}) where {T <: Real}
...
# ╔═╡ a3b9be2d-694e-4a3f-a963-3eda38bc00db
function bellman_update_sweep_parallel!(value_ests::Array{T, N}, ptf::TabularTransitionDistribution{T, ST, RT}, γ::T, sweep) where {T <: Real, ST, RT, N}
	delt = Atomic{T}(zero(T))
	num_updates = Atomic{Int64}(0)
	@threads for args in sweep
		newdelt = bellman_optimal_update!(value_ests, args..., ptf, γ)
		atomic_max!(delt, newdelt)
...
function bellman_update_sweep_sequential!(value_ests::Array{T, N}, ptf::TabularTransitionDistribution{T, ST, RT}, γ::T, sweep) where {T <: Real, ST, RT, N}
	delt = zero(T)
	num_updates = 0
	for args in sweep
		newdelt = bellman_optimal_update!(value_ests, args..., ptf, γ)
		delt = max(delt, newdelt)
		num_updates += 1
	end
	return delt, num_updates
end

# ╔═╡ b422a6e9-4cfa-46aa-860a-9a42c0dcddba
begin
	bellman_update_sweep!(::Val{true}, args::Vararg{Any, N}) where N = bellman_update_sweep_parallel!(args...)
	bellman_update_sweep!(::Val{false}, args::Vararg{Any, N}) where N = bellman_update_sweep_sequential!(args...)
	function bellman_update_sweep!(value_ests::Array{T, N}, ptf::TabularTransitionDistribution, γ::T, sweep; usethreads::Bool = false) where {N, T<:Real}
		bellman_update_sweep!(Val(usethreads), value_ests::Array{T, N}, ptf::TabularTransitionDistribution, γ::T, sweep)
	end
end
...
# ╔═╡ 02056419-b008-4254-bbc1-369ab150b772
begin
	#update the value function as well as the associated ϵ-greedy policy
	function bellman_optimal_trajectory_update!(V::Vector{T}, π::Matrix{T}, π_vec::Vector{T}, i_s::Int64, ptf::TabularTransitionDistribution{T, ST, RT}, γ::T, ϵ::T) where {T <: Real, ST, RT}
		(num_actions, num_states) = size(ptf.state_transition_map)
		x = typemin(T)
		@inbounds @simd for i_a in 1:num_actions
			q = bellman_state_action_value(ptf, i_s, i_a, γ, V)
		...
		make_ϵ_greedy_policy!(π_vec; ϵ = ϵ)
		@inbounds @simd for i_a in 1:num_actions
			π[i_a, i_s] = π_vec[i_a]
		end
		# make_ϵ_greedy_policy!(π, i_s, x; ϵ = ϵ)
		delt = calc_pct_change(V[i_s], x)
		V[i_s] = x
		return delt
	end

	function bellman_optimal_trajectory_update!(V′::Vector{T}, V::Vector{T}, π::Matrix{T}, π_vec::Vector{T}, i_s::Int64, ptf::TabularTransitionDistribution{T, ST, RT}, ϵ::T) where {T <: Real, ST, RT}
		(num_actions, num_states) = size(ptf.state_transition_map)
		x = typemin(T)
		@inbounds @simd for i_a in 1:num_actions
			q = bellman_state_action_value(ptf, i_s, i_a, one(T), V)
			# π[i_a, i_s] = q
			π_vec[i_a] = q
			x = max(x, q)
		end
...
# ╔═╡ a464175d-92c8-4f89-aebb-37e844bbaf2b
begin
	function bellman_update_trajectory!(v_est::Vector{T}, π::Matrix{T}, π_vec::Vector{T}, ptf::TabularTransitionDistribution{T, ST, RT}, γ::T, ϵ::T, sweep::Vector{I}) where {T <: Real, ST, RT, I<:Integer}
		delt = zero(T)
		num_updates = 0
		for i_s in sweep
			newdelt = bellman_optimal_trajectory_update!(v_est, π, π_vec, i_s, ptf, γ, ϵ)
			delt = max(delt, newdelt)
			num_updates += 1
		end
		return delt, num_updates
	...
	function bellman_update_trajectory!(v_est1::Vector{T}, v_est2::Vector{T}, π::Matrix{T}, π_vec::Vector{T}, ptf::TabularTransitionDistribution{T, ST, RT}, ϵ::T, sweep::Vector{I}) where {T <: Real, ST, RT, I<:Integer}
		delt = zero(T)
		num_updates = 0
		for i_s in sweep
			newdelt = bellman_optimal_trajectory_update!(v_est2, v_est1, π, π_vec, i_s, ptf, ϵ)
			num_updates += 1
		end
		vref = v_est2[1]
		v_est2 .-= vref
		delt = maximum(calc_pct_change.(v_est1, v_est2))
		v_est1 .= v_est2
		return delt, vref, num_updates
	end
end

# ╔═╡ 28d2441d-ccb9-46f1-92ce-64d4c099d60a
begin
	function value_iteration_trajectory!(v_est::Vector{T}, π::Matrix{T}, π_vec::Vector{T}, θ::T, ϵ::T, ptf::TabularTransitionDistribution{T}, nmax::Integer, save_history::Bool, sweep::Vector{I}, stop_trigger::Vector{Bool}; make_final_policy::Function = make_greedy_policy, show_message::Bool = true) where {T<:Real, I<:Integer}
		...
		v_est2 = copy(v_est)
		while (delt > θ) && (n <= nmax)
			delt, vref, num_updates = bellman_update_trajectory!(v_est, v_est2, π, π_vec, ptf, ϵ, sweep)
			push!(reward_estimates, vref)
			total_updates += num_updates
			show_message && @info "Completed sweep number $n with a delta of $delt"
			n += 1
			save_history && push!(valuelist, copy(v_est))
		end
		stop_trigger[1] = true
		π = make_final_policy(v_est, ptf, one(T))
	
		(final_value = v_est, reward_estimates = reward_estimates, total_iterations = n, total_updates = total_updates, value_history = valuelist, optimal_policy = π)
	end

	function value_iteration_trajectory!(v_est::Vector{T}, π::Matrix{T}, π_vec::Vector{T}, θ::T, ϵ::T, ptf::TabularTransitionDistribution{T}, γ::T, nmax::Integer, save_history::Bool, sweep::Vector{I}, stop_trigger::Vector{Bool}; make_final_policy::Function = make_greedy_policy, show_message::Bool = true) where {T<:Real, I <: Integer}
		#when there is no discount rate in the arguments, treat this as an average reward problem where the average value is removed from the value function on each step
		delt = typemax(T)
		total_updates = 0
		valuelist = Vector{Vector{T}}()
		...
		n = 1
		while (delt > θ) && (n <= nmax)
			delt, num_updates = bellman_update_trajectory!(v_est, π, π_vec, ptf, γ, ϵ, sweep)
			total_updates += num_updates
			show_message && @info "Completed sweep number $n with a delta of $delt"
			n += 1
			save_history && push!(valuelist, copy(v_est))
		...
		stop_trigger[1] = true
		π = make_final_policy(v_est, ptf, γ)
	
		(final_value = v_est, total_iterations = n, total_updates = total_updates, value_history = valuelist, optimal_policy = π)
	end
end

# ╔═╡ 37ce519f-784e-4613-8e8f-044585f59bac
function form_state_inverse_map(mdp::TabularMDP{<:Real, <:Any, <:Any, P}) where {P<:TabularDeterministicTransition}
	state_inverse_map = [Set{Int64}() for _ in 1:length(mdp.states)]
	for i_s in eachindex(mdp.states)
		@inbounds @simd for i_a in eachindex(mdp.actions)
			i_s′ = mdp.ptf.state_transition_map[i_a, i_s]
			push!(state_inverse_map[i_s′], i_s)
		end
	end
	return state_inverse_map
end

# ╔═╡ 4c117c00-9275-45dc-9d9e-4a21aab9bc50
function form_state_inverse_map(mdp::TabularMDP{<:Real, <:Any, <:Any, <:TabularStochasticTransition})
	state_inverse_map = [Set{Int64}() for _ in 1:length(mdp.states)]
	for i_s in eachindex(mdp.states)
		for i_a in eachindex(mdp.actions)
			v = mdp.ptf.state_transition_map[i_a, i_s]
			for i_s′ in v.nzind
				push!(state_inverse_map[i_s′], i_s)
			end
		end
...
# ╔═╡ cd09438f-5827-4ad4-a3c2-890ee869bcd7
function value_iteration_prioritized!(v_est::Vector{T}, θ::T, mdp::TabularMDP{T, <:Any, <:Any, P}, max_updates::Integer; make_final_policy::Function = make_greedy_policy, show_message = false, kwargs...) where {T<:Real, P <: TabularTransitionDistribution}
	delt = typemax(T)
	num_updates = 0
	v_est2 = copy(v_est)
	#maintains a queue of states ranked by their priority with the largest value first
	priority_queue = PriorityQueue{Int64, T}(Base.Order.Reverse)
	for i_s in eachindex(mdp.states)
		bellman_optimal_update!(v_est2, v_est, i_s, mdp.ptf)
		delt = abs(v_est2[i_s] - v_est[i_s])
		if delt > θ
			priority_queue[i_s] = delt
		end
		num_updates += 1
	end
	...
	#each element is a dictionary whose keys are state indices and who's values are the set of actions that lead to the state who's index is represented by the vector index.
	state_inverse_map = form_state_inverse_map(mdp)
	
	sweep = 1
	while !isempty(priority_queue) && (num_updates < max_updates)
		show_message && @info "After sweep # $sweep, queue has $(length(priority_queue)) states with top element $(first(priority_queue))"
		i_s = dequeue!(priority_queue)
		bellman_optimal_update!(v_est2, v_est, i_s, mdp.ptf)
		num_updates += 1
		delt = abs(v_est2[i_s] - v_est[i_s])
		if delt > θ
		...
		for i_s′ in state_inverse_map[i_s]
			bellman_optimal_update!(v_est2, v_est, i_s′, mdp.ptf)
			delt = abs(v_est2[i_s′] - v_est[i_s′])
			num_updates += 1
			if delt > θ
	...
	π = make_final_policy(v_est, mdp.ptf, one(T))
	(final_value = v_est, total_updates = num_updates, optimal_policy = π)
end

# ╔═╡ c77f6588-a42b-4ec2-b3ee-54e1f237428e
function value_iteration_prioritized!(v_est::Vector{T}, θ::T, mdp::TabularMDP{T, <:Any, <:Any, <:TabularTransitionDistribution}, γ::T, max_updates::Integer; show_message = false, make_final_policy::Function = make_greedy_policy, kwargs...) where {T<:Real}
	delt = typemax(T)
	num_updates = 0

	calc_delt(x1, x2) = abs(x1 - x2)

	#maintains a queue of states ranked by their priority with the largest value first
	priority_queue = PriorityQueue{Int64, T}(Base.Order.Reverse)
	for i_s in eachindex(mdp.states)
		delt = bellman_optimal_update!(v_est, i_s, mdp.ptf, γ; calc_delt = calc_delt)
		if delt > θ
			priority_queue[i_s] = delt
		end
		num_updates += 1
	...
	π = make_final_policy(v_est, mdp.ptf, γ)
	(final_value = v_est, total_updates = num_updates, optimal_policy = π)
end
...
function value_iteration_prioritized(mdp::TabularMDP{T}, args...; init_value::T = zero(T), v_est::Vector{T} = initialize_state_value(mdp; init_value = init_value), θ::T = sqrt(eps(T)), max_updates::Integer = typemax(Int64), kwargs...) where {T<:Real}
	...
	value_iteration_prioritized!(v_est, θ, mdp, args..., max_updates; kwargs...)
end
...
# ╔═╡ ec927347-14c7-46e2-8065-fa30532759d8
#=╠═╡
value_iteration_prioritized_grid_example = value_iteration_prioritized(new_gridworld, value_iteration_γ; make_final_policy = make_greedy_bit_policy)
  ╠═╡ =#

# ╔═╡ 667e6f49-51dc-48cf-8296-d7ad905cd132
# ╠═╡ skip_as_script = true
#=╠═╡
const trajectory_gridworld_mdp = make_stochastic_gridworld(;stepreward = -1f0, termreward = 0f0)
  ╠═╡ =#
...
begin
	...
	function monte_carlo_episode_update!(value_estimates::Vector{T}, states::AbstractVector{I}, rewards::AbstractVector{T}, mdp::TabularMRP{T}, γ::T, averaging_method::AbstractAveragingMethod{T}) where {T<:Real, I<:Integer}
		l = length(states)
		g = zero(T)
		for i in l:-1:1
			g = γ*g + rewards[i]
...
begin
	function td0_policy_prediction!(v_est::Array{T}, mdp::TabularMDP{T}, π, γ::T, α::T, max_episodes::Unsigned, max_steps::Unsigned; i_s0 = initialize_state_index(mdp)) where {T<:Real}
		ep = 1
		step = 0
		i_s = i_s0
		i_a = sample_action(π, i_s)
...
function td0_prediction!(v_est::Vector{T}, mrp::TabularMRP{T}, γ::T, α::T, max_episodes::Unsigned, max_steps::Unsigned; i_s0 = initialize_state_index(mrp)) where {T<:Real}
	ep = 1
	step = 0
	i_s = i_s0
...
td0_prediction(mrp::TabularMRP{T}, γ::Real; α::Real = one(T)/10, max_steps::Integer = 100_000, max_episodes::Integer = typemax(UInt64), kwargs...) where {T<:Real} = td0_prediction!(initialize_state_value(mrp), mrp, T(γ), T(α), Unsigned(max_episodes), Unsigned(max_steps); kwargs...)
...
begin
	function generalized_sarsa!((value_estimates, policies), mdp::TabularMDP{T}, γ::T, α::T, max_episodes::Unsigned, max_steps::Unsigned, value_update!::Function, policy_update!::Function; i_s0 = initialize_state_index(mdp), save_history = false) where {T<:Real}
		ep = 1
		step = 0
		i_s = i_s0
...
#expected update when target and behavior policies are distinct, the behavior policy is used to generate the next action while the value upate uses the policy distribution for the target policy
function double_expected_sarsa_value_update!(q1::Matrix{T}, q2::Matrix{T}, π_target1::Matrix{T}, π_target2::Matrix{T}, π_behavior::Matrix{T}, mdp::TabularMDP{T}, γ::T, α::T, i_s::Integer, i_a::Integer) where T<:Real
	(r, i_s′, i_a′) = sarsa_step(mdp.ptf, π_behavior, i_s, i_a)
	args = if rand() < 0.5
		(q1, q2, π_target1)
	else
...
initialize_afterstate_value(mdp::TabularAfterstateMDP{T}; init_value::T = zero(T)) where {T<:Real} = init_value .* ones(T, length(mdp.afterstates))

# ╔═╡ a7dc4ff8-1ee1-4da0-bba5-de799fdd450a
function make_random_policy(mdp::TabularAfterstateMDP{T}) where {T<:Real}
...
begin
	...
	function bellman_update_sweep!(V::Vector{T}, ptf::TabularTransitionDistribution{T, 1}, γ::T, statesweep, ::Val{true}) where {T <: Real}
		delt = Atomic{T}(zero(T))
		num_updates = Atomic{Int64}(0)
	...
	bellman_update_sweep!(V::Vector{T}, ptf::TabularTransitionDistribution{T, 1}, γ::T, statesweep; usethreads::Bool=false) where {T <: Real} = bellman_update_sweep!(V, ptf, γ, statesweep, Val(usethreads))
end
...
function policy_evaluation!(value_estimate::Array{T}, π, ptf::TabularTransitionDistribution{T}, γ::T; max_updates::Integer = typemax(Int64), θ::T = sqrt(eps(T)), sweep = make_uniform_sweep(value_estimate), kwargs...) where {T<:Real}
	...
	while (delt > θ) && (total_updates <= max_updates)
		delt, num_updates = bellman_update_sweep!(value_estimate, π, ptf, γ, sweep; kwargs...)
		total_updates += num_updates
		iter += 1
	end
...
function value_iteration!(v_est::Array{T, N}, θ::T, ptf::TabularTransitionDistribution{T}, γ::T, nmax::Integer,  save_history::Bool, sweep; show_message = true, make_final_policy::Function = make_greedy_policy, kwargs...) where {T<:Real, N}
	delt = typemax(T)
	total_updates = 0
	valuelist = Vector{Array{T, N}}()
	if save_history
		push!(valuelist, copy(v_est))
	end
	...
	n = 1
	while (delt > θ) && (n < nmax)
		delt, num_updates = bellman_update_sweep!(v_est, ptf, γ, sweep; kwargs...)
		total_updates += num_updates
		show_message && @info "Completed sweep number $n with a delta of $delt"
		n += 1
		save_history && push!(valuelist, copy(v_est))
...
function value_iteration!(v_est::Array{T, N}, θ::T, ptf::TabularTransitionDistribution{T}, nmax::Integer, save_history::Bool, sweep; show_message = true, make_final_policy::Function = make_greedy_policy, kwargs...) where {T<:Real, N}
	#when there is no discount rate in the arguments, treat this as an average reward problem where the average value is removed from the value function on each step
	delt = typemax(T)
	total_updates = 0
	reward_estimates = Vector{T}()
...
begin
	function bellman_differential_update_sweep!(value_ests::Array{T, N}, value_ests2::Array{T, N}, π::Matrix{T}, ptf::TabularTransitionDistribution{T, 2}, sweep, i_s_ref::Integer) where {T <: Real, N}
		delt = zero(T)
		num_updates = 0
		vref = zero(T)
		for (i, args) in enumerate(sweep)
	...
	function bellman_differential_update_sweep!(V::Vector{T}, ptf::TabularTransitionDistribution{T, 1}, statesweep) where {T <: Real}
		delt = zero(T)
		num_updates = 0
		vref = zero(T)
...
function bellman_afterstate_value(ptf::TabularDeterministicTransition{T, 2}, atf::TabularStochasticTransition{T, 1}, i_y::Integer, γ::T, W::Vector{T}, policy_args::Vararg{Any}) where T<:Real
...
function bellman_afterstate_value(ptf::TabularDeterministicTransition{T, 2}, atf::TabularDeterministicTransition{T, 1}, i_y::Integer, γ::T, W::Vector{T}, policy_args::Vararg{Any}) where T<:Real
	i_s′ = atf.state_transition_map[i_y]
	r = atf.reward_transition_map[i_y]
	v′ = bellman_state_value(ptf, i_s′, policy_args...) #afterstate value
	r + γ*v′
end

# ╔═╡ 4ce36bda-2d9d-44b1-8ac6-f36e87fd1bfc
function bellman_update!(W::Vector{T}, i_y::Int64, mdp::TabularAfterstateMDP{T}, γ::T, policy_args...) where {T <: Real}
...
function uniform_bellman_value!(W::Vector{T}, mdp::TabularAfterstateMDP{T}, γ::T, policy_args::Vararg{Any}) where {T <: Real}
	delt = zero(T)
	num_updates = 0
...
function fill_state_action_value!(π::Matrix{T}, i_s::Integer, i_a::Integer, W::Vector{T}, mdp::TabularAfterstateMDP{T}, γ::T) where T<:Real
...
function value_iteration!(W::Vector{T}, θ::T, mdp::TabularAfterstateMDP{T}, γ::T, nmax::Integer; save_history = true) where {T<:Real}
...
begin
	...
	function value_iteration(mdp::TabularMDP{T}, γ::T, args...; kwargs...) where {T<:Real}
		isone(γ) && !any(mdp.terminal_states) && return value_iteration(mdp.ptf, γ, args...; kwargs...)
		value_iteration(mdp.ptf, γ, args...; kwargs...)
	end

	function value_iteration(mdp::TabularMDP{T}, args...; kwargs...) where {T<:Real}
...
# ╔═╡ 36614dac-ccd4-4fce-aac2-7e826c3bbffb
md"""
### Prioritized Sweeping

Certain states are more important to a solution than others and will require more value updates.  The idea in prioritized sweeping is to prioritize updates according to a measure of their urgency, and perform them in order of priority.  A queue is maintained of every state-action pair whose estimated value would change nontrivially if updated, prioritized by the size of the change.  When the top pair in the queue is updated, the effect on each of its predecessor pairs is computed.  If the effect is greater than some small threshold, then the pair is inserted into the queue with the new priority.  In this way the effects of changes are efficiently propagated backward until quiescence.  A full implementation is shown below.
"""

# ╔═╡ fe13ad86-28b0-4a6e-a42a-2f5cda010c71
function prioritized_sweeping_deterministic(mdp::TabularMDP{T}, γ::T; 
	max_episodes = 1000, max_steps = 10000, max_planning_steps = 100, α = one(T)/10, qinit = zero(T), ϵ = one(T)/10, 
	Qinit = initialize_state_action_value(mdp; init_value=qinit), 
	save_history = false,
	...
	π = zeros(T, length(mdp.actions))
	Q = copy(Qinit)
	Q[:, mdp.terminal_states] .= zero(T)
	num_updates = 0

	function update_action!(π, Q, i_s::Integer)
		@inbounds @simd for i in eachindex(π)
			π[i] = Q[i, i_s]
		end
		make_ϵ_greedy_policy!(π; ϵ = ϵ)
		sample_action(π)
	end

	#each element is a dictionary whose keys are state indices and who's values are the set of actions that lead to the state who's index is represented by the vector index.
	state_action_inverse_map = [Dict{Int64, Set{Int64}}() for _ in 1:length(mdp.states)]

	function update_state_action_inverse_map!(inverse_map, i_s′, i_s, i_a)
		if haskey(inverse_map[i_s′], i_s)
			push!(inverse_map[i_s′][i_s], i_a)
		else
			inverse_map[i_s′][i_s] = Set(i_a)
		end
	end
	...
	rewards = Vector{T}()
	steps = Vector{Int64}()
	value_history = Vector{Matrix{T}}()

	save_history && push!(value_history, Q)
	...
	function q_planning!(Q, queue)
		step = 1
		while (step <= max_planning_steps) && !isempty(queue)
			...
			qmax = calculate_qmax(Q, i_s)
			for i_s̄ in keys(state_action_inverse_map[i_s])
				for i_ā in state_action_inverse_map[i_s][i_s̄]
	...
	ep = 1
	optimal_path = false
	while (ep <= max_episodes) && (step <= max_steps) && !optimal_path
		i_s = initialize_state_index(mdp)
		l = 0
		
		while !mdp.terminal_states[i_s] && (step <= max_steps)
			#take policy action based on Q
			i_a = update_action!(π, Q, i_s)
			(r, i_s′) = mdp.ptf(i_s, i_a)
			...
			#update inverse map
			update_state_action_inverse_map!(state_action_inverse_map, i_s′, i_s, i_a)
	...
	return Q, steps, rewards, state_transition_map, reward_transition_map, num_updates, value_history
end
...
begin
	...
	struct StateMDPTransitionDistribution{T <: Real, S, F <: Function} <: AbstractStateTransition{T, 2, S, F}
		...
		function StateMDPTransitionDistribution(step::F, s::S; test_action_index::Integer = 1) where {F<:Function, S}
			...
			@assert promote_type(S, eltype(states)) != Any "There is no common type between the provided state $s and the transition states $states"
	...
	#when used as a functor just apply the step function to the state action pair indices
	(ptf::StateMDPTransitionDeterministic{T, S, F})(s::S, i_a::Integer) where {T<:Real, S, F<:Function} = ptf.step(s, i_a)
end
...
begin
	...
	struct AfterstateStepDistribution{T<:Real, Y, S, F<:Function} <: AbstractAfterstateTransition{T, 1, Y, S, F}
		...
		function AfterstateStepDistribution(step::F, y::Y) where {F<:Function, Y}
			...
			@assert length(rewards) == length(states) == length(probabilities) "The transition vectors do not have consistent lengths"
			@assert typeof(first(rewards)) == typeof(first(probabilities)) "The rewards and probabilities do not have the same numeric type"
			new{eltype(first(rewards)), Y, eltype(states), F}(step)
		end
	end
...
begin
	...
	function make_non_tabular_ptf(mdp::TabularMDP{T, S, A, P}) where {T<:Real, S, A, P<:TabularStochasticTransition{T, 2}}
		d = Dict(begin
	...
	#if the tabular MDP is deterministic then create a deterministic State transition
	function make_non_tabular_ptf(mdp::TabularMDP{T, S, A, P}) where {T<:Real, S, A, P<:TabularDeterministicTransition{T, 2}}		
		function step(s::S, i_a::Integer) 
			i_s = mdp.state_index[s]
			(r, i_s′) = mdp.ptf(i_s, i_a)
			return (r, mdp.states[i_s′])
		end
		StateMDPTransitionDeterministic(step, first(mdp.states))
	end

	#given a tabular MDP, create a non-tabular sampler ptf
	function make_non_tabular_ptf(mdp::TabularMDP{T, S, A, P}) where {T<:Real, S, A, P<:AbstractTabularTransition{T, 2}}
		function step(s::S, i_a::Integer)
			i_s = mdp.state_index[s]
			(r, i_s′) = mdp.ptf(i_s, i_a)
			s′ = mdp.states[i_s′]
...
begin
	...
	function make_non_tabular_ptf(mrp::TabularMRP{T, S, P}) where {T<:Real, S, P<:TabularStochasticTransition{T, 1}}
		d = Dict(begin
	...
	#given a tabular MDP, create a non-tabular sampler ptf
	function make_non_tabular_ptf(mrp::TabularMRP{T, S, P}) where {T<:Real, S, P<:TabularDeterministicTransition{T, 1}}
		d = Dict(begin
			i_s = mrp.state_index[s]
	...
	function make_non_tabular_ptf(mrp::TabularMRP{T, S, P}) where {T<:Real, S, P<:AbstractTabularTransition{T, 1}}
		d = Dict(begin
			i_s = mrp.state_index[s]
			i_s′ = mrp.ptf.state_transition_map[i_s]
			r = mrp.ptf.reward_transition_map[i_s]
...
begin
	struct StateMRP{T<:Real, S, P<:AbstractStateTransition{T, 1}, StateInit, IsTerm<:Function} <: AbstractMRP{T, S, P, StateInit}
		ptf::P
		initialize_state::StateInit #function which provides an initial state index or a list of states or a list of states with associated probabilities
		isterm::IsTerm #function that returns true if a state is terminal and false otherwise
		StateMRP(ptf::P, initialize_state::F1, isterm::F2) where {T<:Real, S, F<:Function, P<:AbstractStateTransition{T, 1, S, F}, F1, F2<:Function} = new{T, S, P, F1, F2}(ptf, initialize_state, isterm)
	end

	convert_init_state(mrp::TabularMRP{<:Real, S, <:Any, <:Integer}) where S = mrp.states[mrp.initialize_state_index]
	convert_init_state(mrp::TabularMRP{<:Real, S, <:Any, <:AbstractVector{<:Integer}}) where S = Vector{S}([mrp.states[i] for i in mrp.initialize_state_index])
	...
	function StateMRP(mrp::TabularMRP{T, S}) where {T<:Real, S}
		...
		initialize_state = convert_init_state(mrp)
...
begin
	...
	struct StateMDP{T<:Real, S, A, P<:AbstractStateTransition, StateInit, IsTerm, ValidAction <:Function} <: AbstractMDP{T, S, A, P, StateInit}
		actions::Vector{A}
		ptf::P
		initialize_state::StateInit #function which provides an initial state or a list of such states or a list of states with associated probabilities
		isterm::IsTerm #function that returns true if a state is terminal and false otherwise or a list of terminal states or a single terminal state
	...
	isterm(termcheck::Function, s::S) where S = termcheck(s)
	isterm(mdp::Union{StateMDP{<:Real, S}, StateMRP{<:Real, S}}, s::S) where S = isterm(mdp.isterm, s)
	isterm(mdp::TabularMDP, i_s::Integer) = mdp.terminal_states[i_s]

	function StateMDP(actions::AbstractVector, ptf::AbstractStateTransition{T, 2, S}, state_init, term_check; is_valid_action::Function = (s, i_a) -> true, action_index = makelookup(actions)) where {T<:Real, S}
		s0 = initialize_state(ptf, state_init)
		isterm(term_check, s0)
		is_valid_action(s0, 1)
		@assert typeof(s0) <: S
		StateMDP(Vector(actions), ptf, state_init, term_check, is_valid_action, action_index)
	end

	StateMDP(actions::AbstractVector, ptf::AbstractStateTransition{T, 2}, initialize_state; kwargs...) where {T<:Real} = StateMDP(actions, ptf, initialize_state, s -> false; kwargs...)

	convert_init_state(mdp::TabularMDP{<:Real, S, <:Any, <:Any, <:Integer}) where S = mdp.states[mdp.initialize_state_index]
	convert_init_state(mdp::TabularMDP{<:Real, S, <:Any, <:Any, <:AbstractVector{<:Integer}}) where S = Vector{S}([mdp.states[i] for i in mdp.initialize_state_index])
	convert_init_state(mdp::TabularMDP{<:Real, S, <:Any, <:Any, <:Set{<:Integer}}) where S = Set{S}([mdp.states[i] for i in mdp.initialize_state_index])
	convert_init_state(mdp::TabularMDP{<:Real, S, <:Any, <:Any, <:SparseVector{T, I}}) where {S, T<:Real, I<:Integer} = (Tuple(mdp.initialize_state_index.nzval), Tuple(i -> mdp.states[i] for i in mdp.initialize_state_index.nzind))
	convert_init_state(mdp::TabularMDP{<:Real, S, <:Any, <:Any, <:Function}) where S = () -> mdp.states[mdp.initialize_state_index()]

	#convert a tabular mdp into a non-tabular one
	function StateMDP(mdp::TabularMDP{T, S}) where {T<:Real, S}
		termstates = Set(view(mdp.states, mdp.terminal_states))
		initialize_state = convert_init_state(mdp)
...
function make_random_policy_distribution(mdp::StateMDP{T}) where {T<:Real} 
	function π(s)
...
begin
	function runepisode!((states, actions, rewards)::Tuple{Vector{Int64}, Vector{Int64}, Vector{T}}, mdp::TabularMDP{T}; i_s0::Integer = initialize_state_index(mdp), π = make_random_policy(mdp), i_a0 = sample_action(π, i_s0), max_steps = Inf) where {T<:Real}
		@assert any(mdp.terminal_states) #ensure that some terminal state exists since episodes are only defined for problems with terminal states
		i_s = i_s0
		l = length(states)
	...
	function runepisode(mdp::TabularMDP{T}; kwargs...) where {T<:Real}
		states = Vector{Int64}()
...
begin
	function policy_iteration!(v_π::Array{T, N}, ptf::TabularTransitionDistribution, γ::T; initial_policy::P = make_random_policy(ptf), max_iterations = 10, save_history = true, eval_kwargs...) where {T<:Real, N, P}
		...
		return (greedy_policy = πgreedy, value_function = v_π, policy_history = π_list, value_history = v_list)
	end
...
begin
	function differential_policy_iteration!(v_π::Array{T, N}, ptf::TabularTransitionDistribution; initial_policy::P = make_random_policy(ptf), max_iterations = 10, save_history = true, eval_kwargs...) where {T<:Real, N, P}
		...
		return (greedy_policy = πgreedy, value_function = v_π, steady_state_distribution = steady_state, average_reward = r, policy_history = π_list, value_history = v_list, reward_history = r_list)
...
#=╠═╡
md"""Number of Policy Iterations: $(@bind policy_iteration_count2 Slider(0:length(differential_policy_iteration_results2.policy_history) .- 1; show_value=true, default = length(differential_policy_iteration_results2.policy_history) - 1))"""
  ╠═╡ =#

# ╔═╡ 617c1610-d670-442f-9770-f7d4503fdf62
#=╠═╡
differential_policy_iteration_results2
  ╠═╡ =#
...
#=╠═╡
md"""Number of Policy Iterations: $(@bind differential_policy_iteration_count2 Slider(0:length(differential_policy_iteration_results.policy_history) .- 1; show_value=true, default = length(differential_policy_iteration_results.policy_history) - 1))"""
  ╠═╡ =#

# ╔═╡ 60e09cc3-3c66-434d-9e58-652de62a3275
#=╠═╡
plot(differential_policy_iteration_results.reward_history, Layout(xaxis = attr(title = "Iterations", tickvals = 1:length(differential_policy_iteration_results.reward_history)), yaxis_title = "Average Reward per Step"))
  ╠═╡ =#

# ╔═╡ e5eaea84-8f7d-4d2f-bb34-01b5bbf98a4b
#=╠═╡
const avg_reward_policy_discounted_values = policy_evaluation_v(differential_gridworld, differential_policy_iteration_results.policy_history[differential_policy_iteration_count2+1], differential_policy_iteration_params.γ)
  ╠═╡ =#
...
function value_iteration_trajectory(mdp::TabularMDP{T}, args::Vararg{Any, M}; initialize_policy::Function = make_random_policy, init_value::T = zero(T), v_est::Vector{T} = initialize_state_value(mdp; init_value = init_value), ϵ::T = one(T)/10, θ::T = sqrt(eps(T)), nmax::Integer = typemax(Int64), save_history = false, kwargs...) where {T<:Real, M}
	...
	stop_trigger = [false]
	value_iteration_trajectory!(v_est, π, π_vec, θ, ϵ, mdp.ptf, args..., 1, false, sweep, stop_trigger)
	stop_trigger = [false]
	sweep_ref = @spawn update_sweep_trajectory!(sweep, mdp, π, stop_trigger)
	out_ref = @spawn value_iteration_trajectory!(v_est, π, π_vec, θ, ϵ, mdp.ptf, args..., nmax-1, save_history, sweep, stop_trigger; kwargs...)
	out = fetch(out_ref)
	n = fetch(sweep_ref)
	return (;out..., trajectory_updates = n, final_sweep = sweep)
end

# ╔═╡ 284eca49-5d0c-40ba-a9b3-7d3422ca8daf
#=╠═╡
const value_iteration_grid_trajectory_example = value_iteration_trajectory(trajectory_gridworld_mdp, 1f0; ϵ = 0.01f0, nmax = 10000, save_history = true)
  ╠═╡ =#
...
sarsa(mdp::TabularMDP{T}, γ::Real; α = one(T) / 10, ϵ = one(T) / 10, max_steps = 100_000, max_episodes = typemax(Int64), init_value = zero(T), q::Matrix{T} = initialize_state_action_value(mdp; init_value = init_value), π = make_random_policy(mdp), value_update! = sarsa_value_update!, kwargs...) where {T<:Real} = generalized_sarsa!(((q,), (π,)), mdp, γ, α, max_episodes, max_steps, value_update!, (π, q, i_s) -> make_ϵ_greedy_policy!(π, i_s, q; ϵ = ϵ); kwargs...) 
...
function expected_sarsa(mdp::TabularMDP{T}, γ::Real; α = one(T) / 10, ϵ = one(T) / 10, max_steps = 100_000, max_episodes = typemax(Int64), init_value = zero(T), q::Matrix{T} = initialize_state_action_value(mdp; init_value = init_value), π_target = make_random_policy(mdp), π_behavior = make_random_policy(mdp), update_behavior_policy! = (π, i_s, q) -> make_ϵ_greedy_policy!(π, i_s, q; ϵ = ϵ), update_target_policy! = update_behavior_policy!, kwargs...) where {T<:Real} 
...
function value_iteration(mdp::TabularAfterstateMDP{T}, γ::T; init_value::T = zero(T), θ = sqrt(eps(T)), nmax=typemax(Int64), save_history = true) where {T<:Real}
	...
	if save_history
		...
		return (final_value = est, optimal_policy = π)
	end
end
...
# ╠═╡ skip_as_script = true
#=╠═╡
value_iteration_grid_example = value_iteration_v(new_gridworld, value_iteration_γ; show_message = false, make_final_policy = make_greedy_bit_policy)
  ╠═╡ =#
...
begin
	value_iteration_q(problem, γ::T; init_value::T = zero(T), v_est::Matrix{T} = initialize_state_action_value(problem; init_value = init_value), kwargs...) where {T<:Real} = value_iteration(problem, γ, v_est; kwargs...)
	value_iteration_q(problem::TabularMDP{T, S, A, P, F}; init_value::T = zero(T), v_est::Matrix{T} = initialize_state_action_value(problem; init_value = init_value), kwargs...) where {T<:Real, S, A, P, F} = value_iteration(problem, v_est; kwargs...)
end
...
begin
	...
	function runepisode!((states, actions, rewards)::Tuple{Vector{S}, Vector{Int64}, Vector{T}}, mdp::StateMDP{T, S}; s0::S = initialize_state(mdp), π::Function = make_random_policy(mdp), i_a0 = π(s0), max_steps = Inf) where {T<:Real, S}
		s = s0
	...
	function runepisode(mdp::StateMDP{T, S}; kwargs...) where {T<:Real, S}
		states = Vector{S}()
		actions = Vector{Int64}()
		rewards = Vector{T}()
		runepisode!((states, actions, rewards), mdp; kwargs...)
	...
	function runepisode!((states, rewards)::Tuple{Vector{S}, Vector{T}}, mrp::StateMRP{T, S}; s0::S = initialize_state(mrp), max_steps = Inf) where {T<:Real, S}
		s = s0
		...
		#note that the terminal state will not be added to the state list
		while !isterm(mrp, s) && (step <= max_steps)
			add_value!(states, s, step)
			(r, s′) = mrp.ptf(s)
			add_value!(rewards, r, step)
			s = s′
	...
	function runepisode(mrp::StateMRP{T, S}; kwargs...) where {T<:Real, S}
		states = Vector{S}()
		rewards = Vector{T}()
...
monte_carlo_policy_prediction_v(args::Vararg{Any}; kwargs...) = monte_carlo_policy_prediction(args..., initialize_state_value; kwargs...)
...
monte_carlo_policy_prediction_q(args::Vararg{Any}; kwargs...) = monte_carlo_policy_prediction(args..., initialize_state_action_value; kwargs...)
...
function monte_carlo_prediction(mrp::TabularMRP{T}, γ::T, num_episodes::Integer; v_est = initialize_state_value(mrp), averaging_method::AbstractAveragingMethod{T} = SampleAveraging(v_est), save_history = false, epkwargs...) where {T<:Real}
	if save_history
		v_history = zeros(T, size(v_est)..., num_episodes)
	end
...
monte_carlo_control_ϵ_soft(mdp::TabularMDP{T}, γ::T, num_episodes::Integer; ϵ::T = one(T)/10, kwargs...) where T<:Real = monte_carlo_control(mdp, γ, num_episodes, mdp -> (;i_s0 = initialize_state_index(mdp),), (π, i_s, q) -> make_ϵ_greedy_policy!(π, i_s, q; ϵ = ϵ); kwargs...)
...
monte_carlo_off_policy_prediction_q(args::Vararg{Any}; kwargs...) = monte_carlo_off_policy_prediction(args..., initialize_state_action_value; kwargs...)
...
function monte_carlo_off_policy_control(mdp::TabularMDP{T}, γ::T, num_episodes::Integer; π_target = make_random_policy(mdp), π_behavior = make_random_policy(mdp), q = initialize_state_action_value(mdp), weights = zeros(T, length(mdp.actions), length(mdp.states)), compare_error = false, value_reference = zeros(T, length(mdp.states)), sampling_method = WeightedImportanceSampling(), kwargs...) where {T<:Real}
	if compare_error
		error_history = zeros(T, num_episodes)
	end
...
begin
	...
	function sample_rollout(s::S, i_a::Integer, mdp::StateMDP{T, S}, π::Function, γ::T; max_steps::Integer = typemax(Int64), transition_kwargs...) where {T<:Real, S}
		step = 0
		g = zero(T)
	...
	sample_rollout(mdp::StateMDP{T, S}, π::Function, γ::T; s0::S = initialize_state(mdp), i_a0::Integer = π(s0), kwargs...) where {T<:Real,S} = sample_rollout(s0, i_a0, mdp, π, γ; kwargs...)
	sample_rollout(mdp::TabularMDP{T}, π::AbstractArray, γ::T; s0::Integer = initialize_state_index(mdp), i_a0::Integer = sample_action(π, s0), kwargs...) where {T<:Real} = sample_rollout(s0, i_a0, mdp, π, γ; kwargs...)
...
begin
	...
	distribution_rollout(mdp::StateMDP{T, S}, π::Function, γ::T; s0::S = initialize_state(mdp), i_a0::Integer = π(s0), usethreads = false, kwargs...) where {T<:Real,S} = distribution_rollout(s0, i_a0, mdp, π, γ, usethreads ? foldxt : foldxl; kwargs...)	
end
...
# ╔═╡ 439a324b-decf-4597-ba86-1280bec8f545
function apply_uct!(action_values::Vector{T}, action_counts::Vector{T}, c::T) where {T<:Real}
	ntot = sum(action_counts)

	#if no samples have been collected yet then every action value is just from the approximation function.  in this case just use the approximated values on their own to select the best action
	iszero(ntot) && return action_values 
...
# ╔═╡ 263b2c0b-066f-46e0-b198-855afaf1642a
md"""
### Basic Distribution MCTS

When I have an MDP that produces a transition distribution, I should be able to perform MCTS with only storing a table of state values.  The action value lookup can be constructed in real time and also I won't have redundant information for states that are revisted.
"""

# ╔═╡ 255f1c34-9724-41d4-a747-25ddc6424eb5
function update_action_values!(action_values::Vector{T}, s::S, mdp::StateMDP{T, S, <:Any, <:StateMDPTransitionDistribution}, γ::T, v_est::Function, V::Dict{S, Tuple{Vector{T}, Vector{T}}}; est_kwargs...) where {T<:Real, S}
	action_visits = V[s][2]
	for i_a in eachindex(action_values)
		...
		elseif iszero(action_visits[i_a])
			...
			v′ = mapreduce(+, eachindex(probabilities)) do i
				s′ = states[i]
				isterm(mdp, s′) && return zero(T)
				haskey(V, s′) && return probabilities[i]*V[s′][1][1]
				v̂ = v_est(mdp, s′, γ; est_kwargs...)
				V[s′] = ([v̂], zeros(T, length(mdp.actions)))
				return probabilities[i]*v̂
			end
			action_values[i_a] = r_avg + γ*v′
		end
	end
	return action_values
end

# ╔═╡ 0888d728-5983-4434-815b-ff803a849ade
function simulate!(V::Dict, mdp::StateMDP{T}, γ::T, v_est::Function, s, depth::Integer, c::T, action_values::Vector, update_tree_policy!::Function, apply_bonus!::Function, sample_state::Bool, step_kwargs::NamedTuple, est_kwargs::NamedTuple) where {T<:Real}
	#if the state is terminal, produce a value of 0
	isterm(mdp, s) && return zero(T)
	...
	#if state is not selected for sampling then just return the estimate we have so far
	!sample_state && return V[s][1][1]

	depth ≤ 0 && return V[s][1][1]

	#replace this part with a way to compute action values looking ahead one step with the mdp
	update_action_values!(action_values, s, mdp, γ, v_est, V; est_kwargs...)
	action_visits = V[s][2]
	...
	r_avg = dot(rewards, probabilities)
	v′ = mapreduce(i -> simulate!(V, mdp, γ, v_est, states[i], depth-1, c, action_values, update_tree_policy!, apply_bonus!, i == i_select, step_kwargs, est_kwargs)*probabilities[i], +, eachindex(probabilities))
	target = r_avg + γ*v′
	
	action_visits[i_a] += one(T)
	δv = (target - V[s][1][1]) / sum(action_visits)
	v = V[s][1][1] + δv
	V[s][1][1] =v
	return v
end
...
function simulate!(visit_counts, Q, mdp::StateMDP{T, S, <:Any, <:StateMDPTransitionDistribution}, γ::T, π_dist!::Function, pscale::T, topk::Integer, s::S, c::T, prior::Vector, v_hold::Vector, v_new::SparseVector, apply_bonus!::Function, step_kwargs::NamedTuple, est_kwargs::NamedTuple, compute_max_value::Function, sample_index::Bool, depth, vest) where {T<:Real, S}
	#if the state is terminal, produce a value of 0
	isterm(mdp, s) && return (zero(T), 1)
...
function simulate2!(tree::Dict, mdp::StateMDP{T, S, <:Any, <:StateMDPTransitionDistribution}, γ::T, π_dist!::Function, pscale::T, topk::Integer, s::S, c::T, prior::Vector, step_kwargs::NamedTuple, est_kwargs::NamedTuple, compute_min_value::Function, compute_max_value::Function, sample_index::Bool, depth, vest) where {T<:Real, S}
	#if the state is terminal, produce a value of 0
	isterm(mdp, s) && return (zero(T), zero(T))
	...
	for i in eachindex(sample_probabilities)
		s′ = transition_states[i]
		possible_improvement = if isterm(mdp, s′)
			zero(T)
...
function monte_carlo_tree_search2(mdp::StateMDP{T, S, <:Any, <:StateMDPTransitionDistribution}, γ::T, s::S, π_dist!::Function, pscale::T, topk::Integer, min_value::T, max_value::T; 
	nsims = 100, 
	c = one(T)/2,
	...
	sim_message = false) where {T<:Real, S}
...
function simulate!(s::S, visit::Bool, tree_values::Dict{S, Tuple{T, Dict{Int64, Tuple{T, T}}}}, mdp::AfterstateMDP{T, S}, γ::T, v_est::Function, depth::Integer, c::T, v_hold, update_tree_policy!, update_tree!, q_hold, apply_bonus!, step_kwargs, transition_kwargs, est_kwargs) where {T<:Real, S}
	#if the state is terminal, produce a value of 0
	isterm(mdp, s) && return zero(T)
...
function simulate!(w::AS, tree_values::Dict{S, Tuple{T, Dict{Int64, Tuple{T, T}}}}, mdp::AfterstateMDP{T, S, AS}, γ::T, v_est::Function, depth::Integer, c::T, v_hold, update_tree_policy!, update_tree!, q_hold, apply_bonus!, step_kwargs, transition_kwargs, est_kwargs) where {T<:Real, S, AS}
...
begin
	#perform action selection within an mdp for a given state s, discount factor γ, and state value estimation function v_est.  v_est must be a function that takes the arguments (mdp, s, γ) and produces a reward of the same type as γ
	function monte_carlo_tree_search(mdp::StateMDP{T, S}, γ::T, v_est::Function, s0::S; 
		depth = 10, 
		nsims = 100, 
		c = one(T), 
	...
	monte_carlo_tree_search(mdp::TabularMDP{T, S}, γ, v_est::Function, s::S; kwargs...) where {T<:Real, S} = monte_carlo_tree_search(StateMDP(mdp), T(γ), v_est, s; kwargs...)
	
	#by default the state value estimator is a rollout with the random policy
	monte_carlo_tree_search(mdp::StateMDP{T, S}, γ::T, s::S; kwargs...) where {T<:Real, S} = monte_carlo_tree_search(mdp, γ, (mdp, s, γ; vest_kwargs...) -> sample_rollout(mdp, make_random_policy(mdp), γ; s0 = s, max_steps = 1_000, vest_kwargs...), s; kwargs...)
end

# ╔═╡ 5c84d3b7-fee0-43cf-ac6f-c6e761c10b99

#perform action selection within an mdp for a given state s, discount factor γ, and state value estimation function v_est.  v_est must be a function that takes the arguments (mdp, s, γ) and produces a reward of the same type as γ
function monte_carlo_tree_search(mdp::StateMDP{T, S, <:Any, <:StateMDPTransitionDistribution}, γ::T, v_est::Function, s::S; 
	depth = 10, 
	nsims = 100, 
	c = one(T), 
	V = Dict{S, Tuple{Vector{T}, Vector{T}}}(), #for each state store its value estimate and a vector of how many times each action has been visited.  the sum of this vector is the total action visits
	...
	for k in 1:nsims
		...
		simulate!(V, mdp, γ, v_est, s, depth, c, action_values, update_tree_policy!, apply_bonus!, true, make_step_kwargs(seed), make_est_kwargs(seed))
	end
	...
	return sample_action(action_values),  action_values, V
end

# ╔═╡ f5e0b84b-32c1-4821-9c06-7d977c5d01ff
#perform action selection within an mdp for a given state s, discount factor γ, and state value estimation function v_est.  v_est must be a function that takes the arguments (mdp, s, γ) and produces a reward of the same type as γ
function monte_carlo_tree_search(mdp::StateMDP{T, S, <:Any, <:StateMDPTransitionDistribution}, γ::T, s::S, π_dist!::Function, pscale::T, topk::Integer; 
	nsims = 100, 
	c = one(T), 
	visit_counts = Dict{S, SparseVector{T, Int64}}(), 
	Q = Dict{S, SparseVector{T, Int64}}(),
...
#perform action selection within an mdp for a given state s, discount factor γ, and state value estimation function v_est.  v_est must be a function that takes the arguments (mdp, s, γ) and produces a reward of the same type as γ
function monte_carlo_tree_search(mdp::AfterstateMDP{T, S}, γ::T, v_est::Function, s::S; 
	depth = 10, 
	nsims = 100, 
	c = one(T), 
...
# ╠═╡ skip_as_script = true
#=╠═╡
monte_carlo_tree_search(mc_gridworld, 0.99f0, initialize_state(mc_gridworld); nsims = 10_000, depth = 1_000, c = 1f0)[3][initialize_state(mc_gridworld)]
  ╠═╡ =#

# ╔═╡ 84790981-a0ea-4680-a656-f591dea83b7e
#=╠═╡
monte_carlo_tree_search(mc_stochastic_gridworld, 0.99f0, initialize_state(mc_gridworld); nsims = 1_000, depth = 1_000, c = 1f0)#[3][mc_gridworld.initialize_state()]
  ╠═╡ =#
...
# ╠═╡ skip_as_script = true
#=╠═╡
show_grid_value(mdp::TabularMDP, Q, name; kwargs...) = show_grid_value(mdp.states, mdp.terminal_states, initialize_state_index(mdp), Q, name; kwargs...)
  ╠═╡ =#

# ╔═╡ 01e452c9-f038-4e37-9586-7ed3f95dc00c
show_grid_probabilities(mdp::TabularMDP, Q, name; kwargs...) = show_grid_probabilities(mdp.states, mdp.terminal_states, initialize_state_index(mdp), Q, name; kwargs...)
...
#=╠═╡
function show_grid_probabilities(states, terminds::BitVector, i_s0::Integer, μ::Vector, name; scale = 1.0, title = "", sigdigits = 2, square_pixels = 20, highlight_state_index = 0)
...
# ╠═╡ skip_as_script = true
#=╠═╡
function show_grid_value(states, terminds::BitVector, i_s0::Integer, Q, name; scale = 1.0, title = "", sigdigits = 2, square_pixels = 20, highlight_state_index = 0)
	width = maximum(s.x for s in states)
	height = maximum(s.y for s in states)
	start = states[i_s0]
	sterms = any(terminds) ? states[terminds] : [GridworldState(0, 0)]
...
# ╔═╡ 7f53e691-24f2-426e-9991-3497afc84c12
#=╠═╡
let
	mdp = make_stochastic_gridworld(;wind= wind_values)
	value_iter = value_iteration_v(mdp, 0.99f0; show_message = false, make_final_policy = make_greedy_policy)
	# π = make_random_bit_policy(mdp)
	π = value_iter.optimal_policy
	output = calculate_μ_episodic(mdp, π; maxiter = 100000)
	show_grid_value(mdp, output.total_visitations, "gjgjgjg"; square_pixels = 40)
end
  ╠═╡ =#
...
# ╠═╡ skip_as_script = true
#=╠═╡
function display_rook_policy(v::AbstractVector{T}; scale = 1.0) where T<:Real
	@htl("""
...
# ╔═╡ 56060ecf-3958-4a55-886b-cec8883dfc27
display_rook_policy(v::AbstractVector{B}; scale = 1.0) where B<:Bool = display_rook_policy(v ./ sum(v))
...
function show_deterministic_gridworld(mdp::TabularMDP, highlight_state_index, grid_action_selection; name = "deterministic_gridworld_transitions", kwargs...)
	...
	show_grid_transitions(mdp.states, mdp.terminal_states, () -> initialize_state_index(mdp), name; highlight_state = s, transition_states = Dict([s′ => 1f0]), action_display = show_selected_action(grid_action_selection), reward_values = [(p = 1, r = r |> Float64)], kwargs...)
end
  ╠═╡ =#
...
# ╠═╡ skip_as_script = true
#=╠═╡
function show_grid_policy(states, i_s0::Integer, terminds, π, name; display_function = display_rook_policy, action_display = rook_action_display, scale = 1.0)
	width = maximum(s.x for s in states)
	height = maximum(s.y for s in states)
	start = states[i_s0]
	sterms = any(terminds) ? states[terminds] : [GridworldState(0, 0)]
...
<div style = "display: flex; justify-content: center; align-items: flex-start;">
	<div>Policy after Iteration $policy_iteration_count$(show_grid_policy(new_gridworld.states, initialize_state_index(new_gridworld), new_gridworld.terminal_states, π_list[policy_iteration_count+1], "policy_iteration_deterministic_gridworld"))</div>
...
@htl("""
<div style = "display: flex; justify-content: center; align-items: flex-start;">
	<div>Policy after Iteration $policy_iteration_count$(show_grid_policy(new_gridworld.states, initialize_state_index(new_gridworld), new_gridworld.terminal_states, differential_policy_iteration_results2.policy_history[policy_iteration_count2+1], "policy_iteration_average_reward_gridworld"))</div>
	<div>Corresponding Differential Value Function$(show_grid_value(new_gridworld, differential_policy_iteration_results2.value_history[policy_iteration_count2+1], "policy_iteration_values", square_pixels = 40))</div>
...
@htl("""
<div style = "display: flex; justify-content: center; align-items: flex-start;">
	<div>Policy after Iteration $episodic_policy_iteration_count$(show_grid_policy(episodic_gridworld.states, initialize_state_index(episodic_gridworld), episodic_gridworld.terminal_states, π_list_episodic[episodic_policy_iteration_count+1], "episodic_policy_iteration_deterministic_gridworld"))</div>
...
<div style = "display: flex; justify-content: space-around; align-items: flex-start;">
	<div>Policy after Iteration $differential_policy_iteration_count2$(show_grid_policy(differential_gridworld.states, differential_gridworld.initialize_state_index, differential_gridworld.terminal_states, differential_policy_iteration_results.policy_history[differential_policy_iteration_count2+1], "differential_policy_iteration_deterministic_gridworld"))</div>
	<div>Differential Value Function$(show_grid_value(differential_gridworld, differential_policy_iteration_results.value_history[differential_policy_iteration_count2+1], "differential_values", square_pixels = 40))</div>
...
# ╔═╡ cb8a0a80-ac58-4b01-9855-2def71368e00
#=╠═╡
@htl("""
<div style = "display: flex; justify-content: center; align-items: flex-start;">
	<div style = "margin: 10px;">Optimal value function found after $(value_iteration_prioritized_grid_example.total_updates) updates $(show_grid_value(new_gridworld, value_iteration_prioritized_grid_example.final_value, "policy_iteration_values", square_pixels = 40))</div>
	<div style = "margin: 10px;">Corresponding greedy policy
	$(show_grid_policy(new_gridworld.states, new_gridworld.initialize_state_index, new_gridworld.terminal_states, value_iteration_prioritized_grid_example.optimal_policy, "value_iteration_deterministic_gridworld"))</div>
</div>
""")
  ╠═╡ =#

# ╔═╡ a0fbb401-2212-42d4-856b-14f2cfd9dc84
#=╠═╡
@htl("""
<div style = "display: flex; justify-content: center; align-items: flex-start;">
	<div style = "margin: 10px;">Optimal value function found after $(value_iteration_grid_trajectory_example.total_iterations) steps $(show_grid_value(trajectory_gridworld_mdp, last(value_iteration_grid_trajectory_example.value_history), "policy_iteration_values", square_pixels = 40))</div>
	<div style = "margin: 10px;">Corresponding greedy policy
	$(show_grid_policy(trajectory_gridworld_mdp.states, trajectory_gridworld_mdp.initialize_state_index, trajectory_gridworld_mdp.terminal_states, value_iteration_grid_trajectory_example.optimal_policy, "policy_iteration_deterministic_gridworld"))</div>
</div>
""")
  ╠═╡ =#
...
<div style = "display: flex; justify-content: center; align-items: flex-start;">
	...
	<div style = "margin: 10px;">Corresponding greedy policy
	$(show_grid_policy(deterministic_gridworld.states, initialize_state_index(deterministic_gridworld), deterministic_gridworld.terminal_states, first(expected_sarsa_test.policies), "sarsa_optimal_policy_gridworld"))</div>
</div>
""")
  ╠═╡ =#
...
function show_mcts_solution(mdp::StateMDP; nsims = 10_000, depth = 1_000, c = 1f0, kwargs...)
	...
	(states, actions, rewards, goal) = runepisode(mdp; π = s -> monte_carlo_tree_search(mdp, 0.99f0, s; nsims = nsims, depth = depth, c = c, visit_counts = visit_counts, Q = Q)[1], s0 = initialize_state(mdp))

	# return(states, Q, visit_counts)
	plot_path(states, GridworldState(8, 4), initialize_state(mdp); kwargs...)
end
  ╠═╡ =#
...
# ╔═╡ 9c7a562d-f375-4821-9fee-e4f974876ed6
#=╠═╡
function show_mcts_solution2(mdp::StateMDP; nsims = 10_000, depth = 1_000, c = 1f0, max_steps = typemax(Int64), kwargs...)
	V = Dict{GridworldState, Tuple{Vector{Float32}, Vector{Float32}}}()

	(states, actions, rewards, goal) = runepisode(mdp; π = s -> monte_carlo_tree_search(mdp, 0.99f0, s; nsims = nsims, depth = depth, c = c, V = V)[1], s0 = initialize_state(mdp), max_steps = max_steps)
...
# ╔═╡ b3426905-4a82-431b-a17f-48153660319d
#=╠═╡
show_mcts_solution2(mc_stochastic_gridworld; xmax = 10, ymax = 7, depth = 10, nsims = 10_000, max_steps = 30, c = 1f0)
  ╠═╡ =#
...
manifest_format = "2.1"
project_hash = "55458aef468ce9f3944152d30b03022ac4961a75"
