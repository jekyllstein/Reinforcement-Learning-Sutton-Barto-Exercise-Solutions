...
# ╔═╡ a80a443a-b9d7-499e-abb0-628053fd6cc1
# ╠═╡ disabled = true
#=╠═╡
...
# ╔═╡ b2def044-1e3d-4b32-a1be-7c1a24fa0507
...
function show_entropy_new_word_options(s::WordleState; nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_entropy_vest, s; nsims = nsims, depth = depth, c = c, apply_bonus! = apply_entropy_puct!)
	rootvisits = [(word = nyt_valid_words[a[1]], visits = a[2]) for a in visits[s]]
	rootvalues = [(word = nyt_valid_words[a[1]], value = a[2]) for a in qs[s]]
	# topvisitinds = sortperm(visits[s0], rev = true)
	# topqinds = sortperm(qs[s0], rev = true)
...
function show_entropy_word_options(;s0 = gamestart, nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_entropy_vest, s0; nsims = nsims, depth = depth, c = c, visit_counts = wordle_entropy_visits, Q = wordle_entropy_qs, possible_indices = entropy_possible_indices, apply_bonus! = apply_entropy_puct!)
	rootvisits = [(word = nyt_valid_words[a[1]], visits = a[2]) for a in visits[s0]]
	rootvalues = [(word = nyt_valid_words[a[1]], value = a[2]) for a in qs[s0]]
	# topvisitinds = sortperm(visits[s0], rev = true)
	# topqinds = sortperm(qs[s0], rev = true)
...
begin
	entropy_prior
	const base_entropy_word_options = show_entropy_word_options(;nsims = 10, c = 1000f0)
	...
	@use_effect([]) do
		...
		schedule(Task() do
			...
			for i in 1:nruns
				...
				set_entropy_word_options(show_entropy_word_options(;nsims = 100, c = 100f0))
				sleep(1)
			end
