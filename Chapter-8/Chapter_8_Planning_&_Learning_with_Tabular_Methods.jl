...
using BenchmarkTools, PlutoProfile

# ╔═╡ d5f3ac61-0696-41aa-a547-24d436ac8d77
using PlutoHooks, Base.Threads

# ╔═╡ b911d430-3519-4259-bf9c-884f940c6001
using SparseArrays
...
# ╔═╡ 95559b56-1d76-4e92-909b-fa792359e43b
using LinearAlgebra
...
Prioritized sweeping also makes use of expected updates to calculate the priority (max function and sweeping through model states) which could be very costly in environments with large numbers of actions.  Also in stochastic environments, our model may not even be accurate for some low probability transitions which would be a waste to consider.  An alternative is to use sample updates which are higher variance individually, but may win out in terms of rate of convergence per computation in environments with high stochasticity and large branching.  In the next sections, we more closely examine the value of expected and sample updates.
"""
...
# ╠═╡ disabled = true
#=╠═╡
show_word_new_options(WordleState([word_index["plant"], word_index["tired"], word_index["story"]], [0x0051, 0x000a, 0x0030]); nsims = 10_000)
  ╠═╡ =#
...
# ╠═╡ disabled = true
#=╠═╡
show_entropy_new_word_options(WordleState([word_index["delta"], word_index["phony"]], [0x000, 0x00de]); nsims = 100_000)
  ╠═╡ =#
...
function simulate!(visit_counts, Q, mdp, γ::T, v_est, s, depth, c::T, v_hold, update_tree_policy!, updateQ!, updateV!, q_hold; stepkwargs...) where T<:Real
	...
	bonus(n::T, ntot::T) = ifelse(iszero(n), typemax(T), sqrt(log(ntot) / n))

	#otherwise, select an action according to UCB selection and backup the result
	ntot = sum(values(visit_counts[s]))
	#all the unvisited states should start with infinite value estimate as per UCB selection
	v_hold .= T(Inf)

	@inbounds @fastmath for i in keys(Q[s])
		#note that the only bonus values computed here are for non zero states
		v_hold[i] = Q[s][i] + c * sqrt(log(ntot)/visit_counts[s][i])
	end
	
	# if ntot != 0
	# 	v_hold .= zero(T)
	# else
	# 	v_hold .= Q[s] .+ c .* bonus.(visit_counts[s], ntot) 
	# end
	...
	q = r + γ*simulate!(visit_counts, Q, mdp, γ, v_est, s′, depth - 1, c, v_hold, update_tree_policy!, updateQ!, updateV!, q_hold; stepkwargs...)
...
#perform a rollout from state s using a policy function π that produces an action selection given a state input
...
#perform a random rollout from state s when no policy function is provided
...
function monte_carlo_tree_search(mdp::MDP{S, A, F, G, H}, γ::T, v_est::Function, s::S; 
	...
	for k in 1:nsims
		simulate!(visit_counts, Q, mdp, γ, v_est, s, depth, c, v_hold, update_tree_policy!, updateQ!, updateV!, q_hold; stepkwargs...)
	end
	
	# v_hold .= Q[s]
	for i in eachindex(v_hold)
		v_hold[i] = get_dict_value(Q[s], i)
	end
...
# ╔═╡ 140e3068-128c-435e-a5bc-2833763b0ab9
md"""
## Wordle Environment
"""

# ╔═╡ 04fbecd2-4193-41c4-b47e-493499263f46
md"""
### Wordle Feedback Calculation
"""

# ╔═╡ 01e0901f-f585-456c-a417-e7799405205d
const EXACT = 0x02

# ╔═╡ 5385b3f6-3c65-4bc1-abb3-f5ca80ec0d46
const MISPLACED = 0x01

# ╔═╡ c8e7cab7-c0fd-47fe-8cb4-3ca9e9c32f45
const MISSING = 0x00

# ╔═╡ 7e1767cb-0b2e-4b53-ad04-ee899b5543aa
...
# ╔═╡ 469140cc-f601-4355-b171-b157537c9d34
...
# ╔═╡ 4bd48e3b-420f-44a8-8026-c45cdb16ab1c
nyt_valid_words[6171]
...
# ╔═╡ 6609227a-7325-4c48-ac9e-29bb328cc605
#words represented as vectors of length 5 where each letter is replaced by its corresponding alphabetic index, e.g. a -> 1
const valid_word_vecs = [SVector{5, UInt8}(letterlookup[word[i]] for i in 1:5) for word in nyt_valid_words]

# ╔═╡ 0b6c8f19-e8aa-455b-a7f9-340cf5246b52
...
# ╔═╡ e4e50dfa-950f-40f0-a202-7aef055699dd
...
# ╔═╡ 0735138f-6e8e-4159-ae33-fa1d0f771389
...
# ╔═╡ 032ecec7-770a-4345-8f0f-0f147f0db1ec
function make_feedback_matrix(word_list, savedata)
	fname = "feedbach_matrix_$(hash(word_list)).bin"
	isfile(fname) && return read_feedback(fname, length(word_list))
	wordvecs = [SVector{5, UInt8}(letterlookup[word[i]] for i in 1:5) for word in word_list]
	...
	for i in 1:l for j in 1:l
		feedback_matrix[i, j] = get_feedback_bytes(wordvecs[i], wordvecs[j]; counts = counts, output=output)
	end end
	if savedata > 0
		write(fname, feedback_matrix)
	end
...
const feedback_matrix = make_feedback_matrix(nyt_valid_words, savedata)

# ╔═╡ 1fc262e7-0ca6-4e8b-9add-0e2a4878cdf6
begin
	#actions should be indices of valid guesses
	const wordle_actions = collect(eachindex(nyt_valid_words))

	struct WordleState
		guess_list::Vector{Int64}
		feedback_list::Vector{UInt16}
	end
	...
	function isterm(s::WordleState) 
		...
		(last(s.feedback_list) == first(feedback_matrix)) && return true
	end
end
...
# ╔═╡ 278b198a-56cb-4843-baf7-86826202872e
feedback_matrix[word_index["shiny"], word_index["phony"]]

# ╔═╡ 8626cae1-c9ad-492e-b46b-74ca36dac2ad
function init_feedback_sets(feedback_matrix)
	l = size(feedback_matrix, 1)
	feedback_sets = Matrix{BitVector}(undef, l, 243)
	v = BitVector(0 for _ in 1:l)
	for i in eachindex(feedback_sets)
		feedback_sets[i] = copy(v)
	end
	return feedback_sets
end

# ╔═╡ 61f2e2c3-f489-4a58-bd19-5c1760c57516
function make_guess_answer_feedback_lookup(feedback_matrix, savedata)
	fname = "guess_feedback_lookup $(hash(feedback_matrix)).bin"
	l = size(feedback_matrix, 1)
	out = BitArray(undef, l, 243, l)
	if isfile(fname)
		f = open(fname)
		read!(f, out)
		close(f)
		...
		for guess_index in 1:l
			for feedback in 1:243
				for answer_index in 1:l
					out[answer_index, feedback, guess_index] = feedback_matrix[guess_index, answer_index] == feedback - 1
				end
			end
		end
		if savedata
...
const guess_feedback_lookup = make_guess_answer_feedback_lookup(feedback_matrix, savedata)

# ╔═╡ 3b491841-de32-4bd8-bae8-1e197bdc0026
function calculate_guess_feedback_distribution(guess_index::Integer, answer_weights::Vector{Float32}; output = zeros(Float32, 243), A = zeros(Float32, length(answer_weights), 243))
	A .= guess_feedback_lookup[:, :, guess_index]
...
function calculate_entropy(p::AbstractVector{T}) where T <: Real
	h = zero(T)
	@fastmath @inbounds @simd for x in p
		h -= ifelse(iszero(x), zero(T), x*log2(x))
	end
	return h
end
...
function eval_guess_entropy(guess_index::Integer, possible_indices::AbstractVector, weights::Vector{Float32}; feedback_probabilities = zeros(Float32, 243), selectweights = zeros(Float32, length(nyt_valid_words)), feedbackinds = zeros(UInt16, length(nyt_valid_words)))
	feedback_probabilities .= 0f0
	l = sum(possible_indices)
	# selectweights= weights[possible_indices]
	if l == length(possible_indices)
		s = sum(weights)
		feedbackinds .= feedback_matrix[guess_index, :]
		for i in 1:l
		...
		selectweights[1:l] .= weights[possible_indices]
		s = 0f0
		@fastmath @inbounds @simd for i in 1:l
			s += selectweights[i]
		end
		# s = sum(selectweights) #this is faster even with allocations
	
		#runs about twice as fast as loop version ~270 microseconds per query with preallocated inputs
		# feedbackinds = view(feedback_matrix, guess_index, possible_indices)
		# feedbackinds = feedback_matrix[guess_index, possible_indices]
		feedbackinds[1:l] .= feedback_matrix[guess_index, possible_indices]
		
		for i in 1:l
			f = feedbackinds[i]
			feedback_probabilities[f+1] += selectweights[i] / s
		end
	end
	calculate_entropy(feedback_probabilities)
end

# ╔═╡ 88d82565-cada-4730-9141-fb9970f94ed9
const testout = zeros(Float32, 243)

# ╔═╡ 97899a9a-298a-451b-af2c-0c9e149ddcf8
const feedback_probability_hold = zeros(Float32, 243)

# ╔═╡ 4ea56644-f6af-4161-a505-743b8f5298ee
const testA = zeros(Float32, length(nyt_valid_words), 243)
	...
	if isfile(fname)
		...
		for guess_index in 1:l
			for answer_index in 1:l
				...
				#include this answer index in the appropriate feedback vector
				feedback_sets[guess_index, f+1][answer_index] = true
			end	
		end
		if savedata > 0
			f = open(fname, "w")
...
function calculate_guess_priors(answer_weights::Vector{Float32}; prior = copy(answer_weights), possible_indices = copy(baseinds), feedback_probabilities = zeros(Float32, 243), selectweights = zeros(Float32, length(nyt_valid_words)), feedbackinds = zeros(UInt16, length(nyt_valid_words)))
	# for guess_index in eachindex(answer_weights)
	# 	entropy = calculate_entropy(calculate_guess_feedback_distribution(guess_index, answer_weights; output = output, A = A))
	# 	prior[guess_index] = entropy
	# end
	# return prior
	for guess_index in eachindex(answer_weights)
		prior[guess_index] = eval_guess_entropy(guess_index, possible_indices, answer_weights; feedback_probabilities = feedback_probabilities, feedbackinds = feedbackinds, selectweights = selectweights)
	end
	return prior
end	
...
# ╔═╡ ab0cffe0-9ebf-49c7-9a33-725126d9810a
get_possible_indices(guess::AbstractString, feedback::Integer) = get_possible_indices(word_index[guess], feedback)

# ╔═╡ 183a1e5f-fd84-4f80-b95a-3c8051cc9b1a
get_possible_indices(guess::AbstractString, feedback::AbstractVector) = get_possible_indices(word_index[guess], convert_bytes(feedback))

# ╔═╡ bad87184-eb30-4169-a940-6abc0a3ce120
...
function wordle_rollout(mdp, s, γ::T; π = s -> rand(mdp.actions), possible_indices = copy(baseinds), stepkwargs...) where T <: Real
	...
	while !mdp.isterm(s)
		...
		elseif num_guesses == 5
			success_rate = Float32(maximum(possible_indices) / num)
			penalty = 1f0 - success_rate
			rtot += (-1f0 - penalty) * γ^(step+1)
			break
		end
		step += 1
	end
...
# ╔═╡ 99268164-fbe3-4c15-9f2d-87c856f2bd4d
small_possible_indices = get_possible_indices([("apple", 4)])
...
function get_possible_indices!(output::BitVector, gamesteps)
	...
	for a in gamesteps
		...
		output .*= v
		# @simd for j in eachindex(output)
		# 	output[j] = output[j] && v[j]
		# end
	end
	return output
end
...
function wordle_hardmode_rollout(mdp, s, γ::T; hard_mode_indices = copy(baseinds), sampling_weights = ones(T, length(hard_mode_indices)), ϵ = one(T)/10, stepkwargs...) where T <: Real
	...
	while !mdp.isterm(s)
		...
		elseif num_guesses == 5
			success_rate = Float32(maximum(hard_mode_indices) / num)
			penalty = 1f0 - success_rate
			rtot += (-1f0 - penalty) * γ^(step+1)
			break
		end
...
function wordle_entropy_policy(s::WordleState, answer_weights::Vector{Float32}; possible_indices = copy(baseinds), kwargs...)
	...
	if num_possible == 1
		findfirst(possible_indices)
	elseif num_possible == 2
		weights = answer_weights[possible_indices]
		guesses = findall(possible_indices)
		guesses[argmax(weights)]
	else
...
function count_expected_remaining(s::WordleState, guess_index::Int64; possible_indices = copy(baseinds))
	get_possible_indices!(possible_indices, s)
	tot = 0f0
	for i in findall(possible_indices)
		f = feedback_matrix[guess_index, i]
		new_possible_indices = get_possible_indices(guess_index, f)
		tot += Float32(dot(possible_indices, new_possible_indices))
	end
	return tot / length(possible_indices)
end	

# ╔═╡ cadf4308-46c2-4763-8949-f83539781bd9
function minimize_expected_remaining(s::WordleState; possible_indices = copy(baseinds))
	min_remaining = Inf32
	a = 0
	for i in eachindex(nyt_valid_words)
		n = count_expected_remaining(s, i; possible_indices = possible_indices)
		min_remaining = min(min_remaining, n)
		a += i*(min_remaining == n)
	end
	return (a, min_remaining)
end

# ╔═╡ 7469f01f-694a-4ad5-8777-9d6518b7d671
#returns a list of possible words given the guess and feedback provided
get_possible_words(args...) = nyt_valid_words[get_possible_indices(args...)]

# ╔═╡ 4739f571-511f-4bd7-b535-e39134e80de5
function count_possible(guess_index, feedback, base_indices)
	...
	# count(new_possible_indices[i] && base_indices[i] for i in eachindex(new_possible_indices))
end
...
function show_word_new_options(s; nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_rollout, s; nsims = nsims, depth = depth, c = c)
...
function show_word_options(;s0 = gamestart, nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_rollout, s0; nsims = nsims, depth = depth, c = c, visit_counts = wordle_normal_visits, Q = wordle_normal_qs, possible_indices = normal_possible_indices)
	rootvisits = [(word = nyt_valid_words[a[1]], visits = a[2]) for a in visits[s0]]
	rootvalues = [(word = nyt_valid_words[a[1]], value = a[2]) for a in qs[s0]]
	# topvisitinds = sortperm(visits[s0], rev = true)
	# topqinds = sortperm(qs[s0], rev = true)
...
function show_entropy_new_word_options(s::WordleState; nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_entropy_vest, s; nsims = nsims, depth = depth, c = c)
	rootvisits = [(word = nyt_valid_words[a[1]], visits = a[2]) for a in visits[s]]
	rootvalues = [(word = nyt_valid_words[a[1]], value = a[2]) for a in qs[s]]
	# topvisitinds = sortperm(visits[s0], rev = true)
	# topqinds = sortperm(qs[s0], rev = true)
...
# ╔═╡ c0eb1a50-b286-410e-8a23-fc9ab83cc422
new_options_out = @spawn show_entropy_new_word_options(WordleState([word_index["trace"]], [0x0000]); nsims = 100_000)

# ╔═╡ 675d9f9b-2270-4171-9316-611360d96a92
fetch(new_options_out)
...
function show_entropy_word_options(;s0 = gamestart, nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_entropy_vest, s0; nsims = nsims, depth = depth, c = c, visit_counts = wordle_entropy_visits, Q = wordle_entropy_qs, possible_indices = entropy_possible_indices)
	rootvisits = [(word = nyt_valid_words[a[1]], visits = a[2]) for a in visits[s0]]
	rootvalues = [(word = nyt_valid_words[a[1]], value = a[2]) for a in qs[s0]]
	# topvisitinds = sortperm(visits[s0], rev = true)
	# topqinds = sortperm(qs[s0], rev = true)
...
begin
	entropy_prior
	const base_entropy_word_options = show_entropy_word_options(;nsims = 10)
	...
	@use_effect([]) do
		...
		schedule(Task() do
			...
			for i in 1:nruns
				...
				set_entropy_word_options(show_entropy_word_options(;nsims = 100))
				sleep(1)
			end
...
function show_ranked_priors(s::WordleState)
	...
	get_possible_indices!(possible_indices, s)
	priors = calculate_guess_priors(wordle_original_weights; possible_indices = possible_indices)
...
entropy_game_stats = @spawn summarystats([wordle_rollout(wordle_stochastic_game, WordleState(), 1f0; π = s -> wordle_entropy_policy(s, wordle_original_weights; prior = testprior, possible_indices = test_possible_indices, feedback_probabilities = testout, selectweights = testselectweights, feedbackinds = testfeedbackinds), possible_indices = test_possible_indices) for _ in 1:1_000])

# ╔═╡ 1447de2e-61f4-491a-9846-647fea84ca4a
...
# ╔═╡ 4b1840f2-ba45-469c-b9a2-7b41f4a83b4f
s0_guess_entropies = calculate_guess_priors(wordle_original_weights; prior = testprior, possible_indices = test_possible_indices, feedback_probabilities = testout, selectweights = testselectweights, feedbackinds = testfeedbackinds)

# ╔═╡ 95db238d-5f58-4322-96fd-d8a44227663a
nyt_valid_words[sortperm(s0_guess_entropies)[1:10]]
...
# ╔═╡ 36b180a7-3f86-46cb-96e3-7f2ab2cd32d0
begin
	const feedback_counts = zeros(Int64, 243)
	const feedback_probabilities = zeros(Float32, 243)
end

# ╔═╡ dd202f16-6a7a-4ca5-b5c5-47d15a21f2b2
const prior_inds = copy(baseinds)

# ╔═╡ 67b5c6bd-7e00-405e-bf56-aa318ac52e78
const prior = zeros(Float32, length(nyt_valid_words))

# ╔═╡ 37bc8a51-2218-40e4-a1c6-7c233affcf2b
#maybe there's a way to sample from the prior without actually calculating it, what if I sample one answer from the distribution of possible remaining answers, then based on that one answer I would just select that word if I already knew that was the answer but if I keep it hidden I can get the distribution of feedback just from sampling all the guesses.  I want to work backwards somehow though so that I find the distribution with the highest entropy directly and then pick one of the guesses that leads to that.  

# ╔═╡ 3b93783b-5710-425e-92b4-1785f37b1dfd
#figure out how many groups of words there are given feedback and a probability weighting of answers
function eval_group(guess_index::Integer, possible_indices::AbstractVector, weights::Vector{Float32}; feedback_counts = zeros(Int64, 243), feedback_probabilities = zeros(Float32, 243), hard_mode_indices = possible_indices)
	feedback_counts .= 0 
	feedback_probabilities .= 0f0
	selectweights = weights[possible_indices]
	s = sum(selectweights) #this is faster even with allocations
	# for (i, b) in enumerate(possible_indices)
	# 	if b
	# 		f = feedback_matrix[guess_index, i]
	...
	#runs about twice as fast as loop version ~270 microseconds per query with preallocated inputs
	feedbackinds = feedback_matrix[guess_index, possible_indices]
	for (i, f) in enumerate(feedbackinds)
		feedback_counts[f+1] += selectweights[i] > 0
		feedback_probabilities[f+1] += selectweights[i] / s
	end
	# feedback_counts[feedbackinds] .+= (weights[possible_indices] .> 0)
	# feedback_probabilities[feedbackinds] .+= weights[possible_indices] ./ s

	expected_value = 0f0
	entropy = 0f0
	expected_hard_mode_guesses = 0f0

	base_counts = all(possible_indices)

	for i in eachindex(feedback_probabilities)
		p = feedback_probabilities[i]
		expected_value += feedback_counts[i]*p
		if p > 0
			entropy-= p*log2(p)
		end

		if base_counts
			n = feedback_set_sums[guess_index, i]
		else
			#possible indices for receiving this feedback with the guess
			new_possible_indices = get_possible_indices(guess_index, i-1)
			#number of hard mode guesses remaining after receiving this feedback
			n = dot(new_possible_indices, hard_mode_indices)
		end
		expected_hard_mode_guesses += n*p
	end
	...
	(win_probability = last(feedback_probabilities), expected_value = expected_value, worstvalue=maximum(feedback_counts), entropy = entropy, hard_mode_count = expected_hard_mode_guesses)
end
...
# ╔═╡ 174b56a0-784b-4ee1-9c7f-3506c23b1e7e
...
# ╔═╡ e734fbfa-d4a0-4610-bb76-584a31fa40b4
const wordle_answer_weights = Float32.(make_pdf_weights(8000)) ./ sum(Float32.(make_pdf_weights(8000)))
...
# ╔═╡ 38ef1340-1e0f-4cf1-b820-105c7f0272fe
@plutoprofview calculate_wordle_prior_guesses(WordleState(); possible_indices = prior_inds, feedback_counts = feedback_counts, feedback_probabilities = feedback_probabilities, prior = prior)

# ╔═╡ a29f899c-3214-4e2e-8c09-a828c96fb5e1
begin
	const game_evals = Dict{Tuple{BitVector, Vector{Float64}, BitVector}, NamedTuple}() 
	
	function eval_guesses(possible_indices::AbstractVector, weights::Vector{Float32}, hard_mode_indices::AbstractVector)
		k = (possible_indices, weights, hard_mode_indices)
		haskey(game_evals, k) && return game_evals[k]
		inds = Iterators.filter(i -> possible_indices[i], eachindex(possible_indices))
		weightsum = sum(weights[possible_indices])
		...
		guessinds = eachindex(nyt_valid_words)
		feedback_counts = zeros(Int64, 243)
		feedback_probabilities = zeros(Float32, 243)
		results = [eval_group(i, possible_indices, weights; feedback_counts = feedback_counts, feedback_probabilities = feedback_probabilities, hard_mode_indices = hard_mode_indices) for i in guessinds]
		guesses = [(guess = nyt_valid_words[i],  entropy = results[i].entropy,  expected_value = results[i].expected_value, win_probability = results[i].win_probability, worst_value = results[i].worstvalue, hard_mode_guess = hard_mode_indices[i], remaining_hardmode = results[i].hard_mode_count) for i in guessinds]
		out = (ranked_answers = ranked_answers, guesses = guesses)
		game_evals[k] = out
	end
	
	function eval_guesses(gamesteps; numcommon = lastindex(nyt_valid_words), weights = wordle_answer_weights, undosteps = Vector{Tuple{String, Vector{Int64}}}()) 
