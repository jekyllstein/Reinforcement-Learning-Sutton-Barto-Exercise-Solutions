...
# ╠═╡ disabled = true
#=╠═╡
fetch(entropy_trace_game_stats)
  ╠═╡ =#
...
function evaluate_game_stats(π, n)
...
function show_entropy_new_word_options(s::WordleState; nsims = 100, depth = 10_000, c::T = 1f0) where T <: Real
	wordindex, visits, qs = monte_carlo_tree_search(wordle_stochastic_game, 1f0, wordle_entropy_vest, s; nsims = nsims, depth = depth, c = c, apply_bonus! = apply_entropy_uct_plus!)
	rootvisits = [(word = nyt_valid_words[a[1]], visits = a[2], values = qs[s][a[1]]) for a in visits[s]]
	rootvalues = [(word = nyt_valid_words[a[1]], value = a[2], visits = visits[s][a[1]]) for a in qs[s]]
	# topvisitinds = sortperm(visits[s0], rev = true)
	# topqinds = sortperm(qs[s0], rev = true)
...
function wordle_entropy_π(s; start_guess = word_index["lions"])
	if s == test_game_start
		start_guess
	else
